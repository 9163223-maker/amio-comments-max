...
const RUNTIME = 'CC8.3.10-BOT-STARTED-NULLSAFE';
const MAX_POSTS = 8;
...
function find(value, predicate, depth = 6, seen = new Set()) { if (!value || depth < 0 || typeof value !== 'object' || seen.has(value)) return null; seen.add(value); if (predicate(value)) return value; for (const item of (Array.isArray(value) ? value : Object.values(value))) { const found = find(item, predicate, depth - 1, seen); if (found) return found; } return null; }
function updateType(update = {}) { return clean(update && (update.update_type || update.type || update?.data?.update_type || update?.data?.type)).toLowerCase(); }
function isOfficialBotStarted(kind = '') { return clean(kind) === 'bot_started'; }
function message(update = {}) { return update?.message || update?.data?.message || update?.callback?.message || update?.data?.callback?.message || find(update, (x) => x && typeof x === 'object' && (x.body?.text || x.text || x.body?.caption || x.caption || x.body?.attachments || x.attachments || x.photo || x.image || x.file) && (x.recipient || x.sender || x.message_id || x.id), 6) || null; }
...
function hasUrl(value = '') { return /https?:\/\/\S+/i.test(clean(value)); }
...
function postTitle(post = {}) { const txt = clean(post?.originalText || post?.postText || post?.text || post?.caption || ''); if (txt) return short(txt, 58); const attachments = arr(post?.sourceAttachments || post?.attachments); if (attachments.length) return attachments.some((item) => /image|photo/i.test(clean(item.type))) ? 'Пост с фото' : 'Пост с вложением'; return 'Пост без текста'; }
...
function channelsFromPosts(user = '') { const map = new Map(); allPosts(user).forEach((post) => { const id = clean(post.channelId); const title = channelTitleFromPost(post); if (!id || /^global$/i.test(title) || map.has(id)) return; map.set(id, title || 'Канал без названия'); }); return Array.from(map.entries()).map(([channelId, title]) => ({ channelId, title })).slice(0, 12); }
...
async function sendFreshScreen(config, update, msg = {}, screen, uid = '') { const cid = chatId(msg); const user = clean(uid || userId(update, null, msg)); await deleteActiveAdminScreens(config, user); const result = await max.sendMessage({ botToken: config.botToken, userId: cid ? '' : user, chatId: cid, text: screen.text, attachments: screen.attachments, notify: false }); rememberAdminScreen(user, resultMessageId(result)); return result; }
async function sendNativeStartScreen(config, update, uid = '', msg = {}) { const user = clean(uid || userId(update, null, null)); if (!user) return { ok: false, skipped: true, reason: 'user_id_missing' }; clearActiveFlows(user); const screen = menu.mainScreen ? menu.mainScreen() : { text: '🐋 АдминКИТ\n\nПанель управления MAX-каналом.\nВыберите раздел.', attachments: [] }; const result = await sendFreshScreen(config, update, msg || {}, screen, user); const sentMessageId = resultMessageId(result); walkthroughTrace.log('native_start.sent', { userId: user, sentMessageId, updateType: updateType(update), runtimeVersion: RUNTIME }); return { ok: true, sentMessageId }; }
...
function createCleanBot(legacy) {
  ...
  return { handleWebhook: async function handleWebhookWithChannelFirstPostPicker(req, res, config) {
    ...
    if (isOfficialBotStarted(kind)) {
      const uid = userId(update, null, null);
      walkthroughTrace.log('native_start.official_bot_started', { userId: uid, updateType: kind, runtimeVersion: RUNTIME });
      const result = await sendNativeStartScreen(config, update, uid, {});
      return res.status(200).json({ ok: true, handledBy: RUNTIME, action: 'official_bot_started_menu', ...result });
    }
    ...
    const mediaInput = updateHasMedia(update);

    if (!real && msg && !isChannelMessage(msg) && (isSlashCommand(incomingText) || isStartText(incomingText))) clearActiveFlows(uid);
    if (!real && msg && !isChannelMessage(msg) && (incomingText || mediaInput) && !isSlashCommand(incomingText)) {
