...
const RUNTIME = 'CC6.5.9.8-SAFE-CORE-PARSER-PATCH-V2';
const MARKER = '__ADMINKIT_SAFE_CORE_PARSER_PATCH_598__';
const OLD_MARKER_597 = '__ADMINKIT_SAFE_CORE_PARSER_PATCH_597__';
...
function safeBrowserMarkerLine() {
...
function patchAppJsText(text) {
  ...
  // Repair the broken CC6.5.9.7 marker. A bare identifier at the top of app.js throws
  // ReferenceError in the browser and leaves the mini-app as an empty pale screen.
  body = body.replace(new RegExp(`(?:^|\\n)\\s*${OLD_MARKER_597};\\s*\\n?`, 'g'), '\n');

  if (!body.includes(MARKER)) {
    body = `${safeBrowserMarkerLine()}\n` + body;
  }
  ...
  if (!body.includes('const legacyKey = extractLegacyCommentKey(decoded);\n  if (legacyKey) return `ck:${legacyKey}`;')) {
    body = body.replace(
  ...
  return { body, changed: body !== before, alreadyPresent: before.includes(MARKER) && !before.includes(OLD_MARKER_597) };
}
...
function selfChecks(text) {
  ...
  return {
    ...
    oldBrokenMarkerRemoved: !body.includes(`${OLD_MARKER_597};`),
    browserMarkerIsSafe: body.includes(safeBrowserMarkerLine()),
...
function patchExpressDebug() {
  if (Module.__adminkitSafeCoreParserPatch598) return;
  Module.__adminkitSafeCoreParserPatch598 = true;
  const previousLoad = Module._load;
  Module._load = function patchedLoad(request, parent, isMain) {
    const loaded = previousLoad.apply(this, arguments);
    try {
      if (String(request) === 'express' && loaded && !loaded.__adminkitSafeCoreParserPatch598Wrapped) {
        function wrappedExpress(...args) {
          const app = loaded(...args);
          if (app && !app.__adminkitSafeCoreParserPatch598Route) {
            app.__adminkitSafeCoreParserPatch598Route = true;
            app.get('/debug/safe-core-parser-live', (req, res) => {
              ...
              res.json({ ok: true, runtimeVersion: RUNTIME, marker: MARKER, oldMarker: OLD_MARKER_597, status, fileHasMarker, checks });
            });
          }
          return app;
        }
        ...
        wrappedExpress.__adminkitSafeCoreParserPatch598Wrapped = true;
