...
// CC7.5.0 Clean Menu Core
// Purpose: keep the proven comments core untouched, but replace the temporary Archive Lite
// menu injection with one direct /start menu path and a filtered archive list.
// No maxApi menu injection, no overlay, no floating hints, no external /app comments links.

const RUNTIME = 'CC7.5.0-CLEAN-MENU-CORE';
const MARKER = '__ADMINKIT_CC7_5_0_CLEAN_MENU_CORE__';
...
function getSenderUserId(message) {
  return String(message?.sender?.user_id || message?.sender?.id || message?.user_id || message?.from?.id || '').trim();
}

function getRecipientChatId(message) {
  return String(message?.recipient?.chat_id || message?.recipient?.id || message?.chat_id || message?.chat?.id || '').trim();
}
...
async function sendOrEdit({ config, message, text, attachments, edit = false }) {
...
async function sendMainMenu({ config, message, edit = false }) {
  return sendOrEdit({ config, message, edit, text: buildMainMenuText(), attachments: buildMainMenuKeyboard() });
}

function postPreview(post = {}, fallback = 'Пост без текста') {
  return truncateText(post?.originalText || post?.title || post?.postTitle || post?.postId || fallback, 80);
}

function isRealChannelId(value = '') {
  return /^-?\d{5,}$/.test(String(value || '').trim());
}

function isRealPostId(value = '') {
  return /^\d{8,}$/.test(String(value || '').trim());
}

function isTechnicalPost(post = {}) {
  const haystack = [
    post?.commentKey,
    post?.channelId,
    post?.postId,
    ...
    post?.originalText,
    post?.raw?.title,
    post?.raw?.originalText,
    post?.raw?.text
  ].map((v) => String(v || '').toLowerCase()).join(' ');

  return /\b(sp\d+|global|stress|debug|selftest|post\s*zero|postzero|dummy|fixture)\b/.test(haystack)
    || String(post?.channelId || '').trim() === '-stress'
    || String(post?.postId || '').trim().startsWith('sp');
}
...
function getStoredPostsForArchive(channelId = '') {
  ...
  return getPostsList()
    ...
    .filter((post) => String(post?.commentKey || '').includes(':'))
    .filter((post) => isRealChannelId(post?.channelId))
    .filter((post) => isRealPostId(post?.postId || post?.messageId))
    .filter((post) => !isTechnicalPost(post))
    .filter((post) => !ch || String(post.channelId || '').trim() === ch)
    .filter((post) => {
      const key = `${String(post.channelId || '').trim()}:${String(post.postId || post.messageId || '').trim()}`;
      if (seen.has(key)) return false;
      seen.add(key);
      return true;
    })
    .sort((a, b) => Number(b?.createdAt || b?.patchedAt || b?.updatedAt || 0) - Number(a?.createdAt || a?.patchedAt || a?.updatedAt || 0));
}
...
function buildArchiveMainText() {
  ...
  return [
    ...
    '',
    'Lite-уровень уже доступен: бот хранит snapshot реальных постов, которые проходили через АдминКИТ.',
    `Постов в базе Lite: ${count}`,
    '',
    'Что можно сейчас:',
    ...
    '',
    'Технические записи, stress/debug/global и Post Zero скрыты из пользовательского архива.',
    '',
    'PRO-уровень будет хранить медиа в отдельном хранилище АдминКИТ.',
...
function buildArchiveListText(page = 0) {
  ...
  if (!slice.length) return ['📚 Посты в базе Lite', '', 'Сохранённых пользовательских постов пока нет.', 'Служебные записи и тестовый мусор скрыты.'].join('\n');
...
function buildPostCardText(post = {}) {
  ...
  return [
    ...
    `Post ID: ${post.postId || post.messageId || 'не указан'}`,
    `Комментариев: ${getComments(post.commentKey || '').length}`,
    ...
    '',
    'Lite-восстановление попробует перепубликовать текст и MAX-вложения из snapshot. Для гарантированного восстановления медиа нужен PRO-архив.'
  ].join('\n');
}
...
function buildProInfoText() {
  return [
    ...
    '',
    'В текущем Lite-режиме сохраняются snapshot текста, ссылок, форматирования и служебные данные MAX-вложений.'
  ].join('\n');
}
...
async function republishLitePost({ config, commentKey }) {
  ...
  if (!post?.channelId || !isRealChannelId(post.channelId)) return { ok: false, error: 'post_or_channel_missing' };
  if (!isRealPostId(post.postId || post.messageId) || isTechnicalPost(post)) return { ok: false, error: 'technical_or_invalid_post_hidden' };
...
async function handleCleanMenuCallback(update, config) {
  ...
  if (action === 'admin_section_archive') {
    await sendOrEdit({ config, message, edit: true, text: buildArchiveMainText(), attachments: buildArchiveMainKeyboard() });
    return { ok: true, action };
  }
  if (action === 'archive_lite_list') {
    const page = Math.max(0, Number(payload.page || 0));
    await sendOrEdit({ config, message, edit: true, text: buildArchiveListText(page), attachments: buildArchiveListKeyboard(page, payload.mode || '') });
    return { ok: true, action, page };
  }
  if (action === 'archive_post_card') {
    const { getPost } = require('./store');
    const post = getPost(String(payload.commentKey || ''));
    if (!post || isTechnicalPost(post) || !isRealPostId(post.postId || post.messageId) || !isRealChannelId(post.channelId)) {
      await sendOrEdit({ config, message, edit: true, text: 'Snapshot не найден или скрыт как служебная запись. Выберите другой пост.', attachments: buildArchiveListKeyboard(0, payload.mode || '') });
      return { ok: true, action: 'archive_post_card_hidden' };
    }
    await sendOrEdit({ config, message, edit: true, text: buildPostCardText(post), attachments: buildPostCardKeyboard(post.commentKey || payload.commentKey || '') });
    return { ok: true, action };
  }
  if (action === 'archive_pro_info') {
    await sendOrEdit({ config, message, edit: true, text: buildProInfoText(), attachments: [{ type: 'inline_keyboard', payload: { buttons: archiveFooterRows() } }] });
    return { ok: true, action };
  }
...
function patchBotWebhook() {
  ...
  if (!bot || bot.__adminkitCleanMenuCorePatched || typeof bot.handleWebhook !== 'function') return { ok: false, reason: 'bot_handleWebhook_missing' };
  const original = bot.handleWebhook;
  bot.handleWebhook = async function adminkitCleanMenuCoreHandleWebhook(req, res, config) {
    try {
      ...
      if (type === 'message_callback') {
        const handled = await handleCleanMenuCallback(update, config || {});
        if (handled) return res.status(200).json({ ok: true, cleanMenuCore: handled });
      }
      if (message && /^\/start(?:\s|$)/i.test(text)) {
        await sendMainMenu({ config: config || {}, message, edit: false });
        return res.status(200).json({ ok: true, cleanMenuCore: { action: 'start_main_menu' } });
      }
    } catch (error) {
      console.error('[clean-menu-core] failed:', error?.message || error, error?.data || '');
    }
    return original.call(this, req, res, config);
  };
  bot.__adminkitCleanMenuCorePatched = true;
  return { ok: true, patched: 'bot.handleWebhook', policy: 'single_start_and_archive_callback_intercept_only' };
}

function install() {
  if (global[MARKER]) return { ok: true, already: true, runtimeVersion: RUNTIME, marker: MARKER };
  global[MARKER] = true;
  const botPatch = patchBotWebhook();
  return {
    ok: true,
    runtimeVersion: RUNTIME,
    marker: MARKER,
    policy: 'clean_start_main_menu_archive_lite_filtered_no_maxapi_menu_injection',
    botPatch,
    features: ['direct_start_main_menu', 'archive_menu_button', 'filtered_lite_snapshot_list', 'lite_republish_attempt', 'pro_placeholder'],
    hiddenArchiveRecords: ['sp*', 'global', 'stress', 'debug', 'selftest', 'Post Zero']
  };
}

module.exports = { install, RUNTIME, MARKER, getStoredPostsForArchive, isTechnicalPost };
