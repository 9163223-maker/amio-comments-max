...
async function coreCanarySend(req) { try { const body = await readRequestJson(req); const params = mergeRequestParams(req, body); const canary = require('./src/core/coreCanaryWebhook'); const config = require('./config'); const dryRun = pickParam(params, 'dryRun', 'dryrun', 'dry_run') ?? '0'; const args = { route: String(pickParam(params, 'route') || 'main.home'), adminId: String(pickParam(params, 'adminId', 'admin_id', 'userId', 'user_id') || ''), chatId: String(pickParam(params, 'chatId', 'chat_id') || ''), messageId: String(pickParam(params, 'messageId', 'message_id', 'activeMessageId', 'active_message_id') || ''), dryRun, updateType: String(pickParam(params, 'updateType', 'update_type') || 'message_callback'), text: String(pickParam(params, 'text') || '') }; const result = await canary.manualSend(args, config); return { ...result, diagnostics: { originalUrl: String(req.originalUrl || ''), url: String(req.url || ''), receivedQuery: req.query || {}, receivedBodyKeys: Object.keys(body || {}), parsedParams: params, dryRunResolved: String(dryRun), adminIdResolved: args.adminId, routeResolved: args.route } }; } catch (e) { return { ok: false, error: e?.message || String(e), runtimeVersion: 'core-canary-send-route-wrapper' }; } }
...
function compactDebug() {
  ...
  return { ok: problems.length === 0, runtimeVersion: RUNTIME, displayVersion: 'CC7.5.25', sourceMarker: SOURCE, generatedAt: new Date().toISOString(), checks, problems, expectedUi: { mainMenuButton: '🎁 Подарки / Лид-магниты', leadFlowTitle: '🎁 Лид-магниты — шаг ...', managerButtons: ['Добавить новый лид-магнит', 'Изменить лид-магнит 1', 'Удалить лид-магнит 1'] }, core: { runtimeVersion: core.runtimeVersion || '', activeInProduction: core.activeInProduction === true, sections: core.sections || [], constraints: core.constraints || {}, delivery: core.delivery || coreAdapter, canaryWebhook: coreCanary }, note: 'Короткий debug. Полный legacy: /debug/cc7-full или /debug/cc7?full=1. Core: /debug/core, /debug/core-render, /debug/core-update, /debug/core-deliver, /debug/core-canary, /debug/core-canary-preview, /debug/core-canary-send' };
}
function coreDebug() { const core = coreRuntimeInfo(); const coreCanary = coreCanaryInfo(); const problems = []; if (core.ok !== true) problems.push(core.error || 'core selfTest failed'); if (core.activeInProduction === true) problems.push('core неожиданно включён в production'); if (coreCanary.ok !== true) problems.push(coreCanary.error || 'core canary webhook selfTest failed'); if (coreCanary.safety?.supportsManualCanarySend !== true) problems.push('core manual send selfTest failed'); return { ok: problems.length === 0, debugType: 'adminkit-core-short', generatedAt: new Date().toISOString(), productionRuntime: RUNTIME, productionStillLegacyLayered: true, core, coreCanaryWebhook: coreCanary, problems, nextMigrationStep: 'проверить /debug/core-canary-send?dryRun=1, затем включать ADMINKIT_CORE_SEND_ENABLED только для canary admin' }; }
function fullDebug() { return { ok: true, runtimeVersion: RUNTIME, sourceMarker: SOURCE, marker: MARKER, installedAt, policy: 'lead_wording_preload_patch_old_gift_labels_no_longer_visible', publicApp: fileInfo('public/app.js', 'CC7.5.6-PUBLIC-APP-COMMENT-UI-SEND-GUARD'), appOnepass: fileInfo('public/app-onepass.js', 'CC7.5.6-COMMENT-UI-SEND-GUARD'), appJsOverride: { ok: false, removed: true }, hardRootFile: fileInfo('menu-v3-hard-root.js', 'adminkit-admin-flows-7525'), adminFlow: adminFlowInfo(), core: coreRuntimeInfo(), coreMaxSendAdapter: safe('maxSendAdapter', () => require('./src/core/maxSendAdapter').selfTest()), coreCanaryWebhook: coreCanaryInfo(), postPatcher: postPatcherInfo(), buildInfo: readBuildInfo(), loadedLayers, audit: { commentsCore: 'unchanged from accepted CC7.5.6', buttonsCore: 'unchanged from accepted CC7.5.16+', leadMagnetsCore: 'CC7.5.25 patches visible wording before legacy/base modules are imported', adminkitCore: 'ADMINKIT-CORE-1.13 exists with MAX send adapter, isolated canary webhook and manual canary send query fix; production runtime is still legacy CC7.5.25', expected: ['main menu shows Подарки / Лид-магниты', 'Core delivery preview available at /debug/core-deliver and does not send messages', 'Core canary preview available at /debug/core-canary-preview', 'Core manual canary send available at /debug/core-canary-send', 'Core canary webhook path exists but does not auto-register in MAX'], stillLayered: true, optimizationNote: 'after canary acceptance merge admin-flow wrappers 7510-7525 into one clean core module' }, commentOpenStateRoute: commentRouteInfo(), generatedAt: Date.now() }; }
