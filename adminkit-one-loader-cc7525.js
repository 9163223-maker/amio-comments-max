...
function installRoutes(app) {
  ...
  app.get('/debug/cc7', (req, res) => {
    ...
    res.json({ ok: true, runtimeVersion: RUNTIME, sourceMarker: SOURCE, marker: MARKER, installedAt,
      ...
      adminFlow: (() => { try { return require('./adminkit-admin-flows-7525').selfTest(); } catch (e) { return { ok: false, error: e?.message || String(e) }; } })(),
      ...
      audit: {
        ...
        expected: ['main menu shows Подарки / Лид-магниты', 'manager buttons show лид-магнит, not подарок', 'Start reset remains from 7.5.24', 'step 4 -> 5 cleanup remains from 7.5.23'],
        stillLayered: true,
        optimizationNote: 'after acceptance merge admin-flow wrappers 7510-7525 into one clean module'
      },
      commentOpenStateRoute: require('./routes/commentOpenState').selfTest(), generatedAt: Date.now() });
  });
  app.get(['/debug/ping', '/debug/version', '/debug/build-info'], (req, res) => { noCache(res); res.json({ ok: true, runtimeVersion: RUNTIME, displayVersion: 'CC7.5.25', sourceMarker: SOURCE, buildInfo: readBuildInfo(), loadedLayers, generatedAt: Date.now() }); });
  return app;
}
