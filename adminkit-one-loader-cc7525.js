...
function compactDebug() {
  ...
  return {
    ...
    note: 'Короткий debug. Полный: /debug/cc7-full или /debug/cc7?full=1'
  };
}
function fullDebug() {
...
function installRoutes(app) {
  ...
  app.get(['/debug/ping', '/debug/version', '/debug/build-info'], (req, res) => { noCache(res); res.json({ ok: true, runtimeVersion: RUNTIME, displayVersion: 'CC7.5.25', sourceMarker: SOURCE, buildInfo: readBuildInfo(), layers: layerSummary(), generatedAt: new Date().toISOString() }); });
  return app;
}
