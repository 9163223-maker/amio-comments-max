...
const RUNTIME = 'HARD-V3-ADMIN-MENU-1.4-FULL-BANNER';
...
async function selfTestAsync(id=''){ const bad=[]; for(const r of ROUTES){ try{ const s=await renderAsync(r,id,{}); if(!s?.text||!Array.isArray(s.attachments)) bad.push(r); }catch(e){ bad.push(r+':'+e.message); } } return {ok:bad.length===0,runtimeVersion:RUNTIME,checked:ROUTES.size,badRoutes:bad,readySections:['main','channels','comments'],banner:'text_button_link_scope_place_preview_reset',patcherTouched:false,commentsUiTouched:false,postgresUsed:true}; }
function selfTest(){ return {ok:true,runtimeVersion:RUNTIME,mainButtons:2,sections:['channels','comments'],banner:'full_controls',patcherTouched:false,commentsUiTouched:false}; }
