...
async function runProductionRouteProbe() {
  ...
  try {
    ...
    async function runVariant(name, urlUpdate) {
      ...
      const repeatStaleResult = savePayload.action ? await drive(bot, callbackUpdateWithPayload(savePayload, previewSend?.mid || previewSend?.messageId || 'preview-msg', 'idempotent-repeat')) : null;
      ...
      const saveCallbackOk = Boolean(savePayloadCurrentOk && saveResult && saveResult.screenId && saveRouteTraceOk && saveDraftTraceOk && (patchCalls.length > 0 || /уже сохранена|already/i.test(saveVisibleText)) && saveVisibleSuccessOk && savePreviewClosedAfterSendOk);
      const ok = step1Ok && step2Ok && step3Ok && step3AfterUrl && sends >= 2 && cleanupTouched && sameOwner && normalizedUrlOk && saveCallbackOk && repeatSaveIdempotentOk && !finalState.buttonsWizardEditFailedAt;
      const step3Text = (sendCalls.slice(beforeUrlSendCount).find((call) => /Шаг 3\/3/.test(call.text)) || {}).text || '';
      return { name, ok, payload, step1Ok, step2Ok, step3Ok, step3AfterUrl, sends, cleanupTouched, requiredClosedWizardMessageIds, closedWizardMessageIds, deletedWizardMessageIds, closedOrDeletedWizardMessageIds, allRequiredWizardMessagesClosed, sameOwner, normalizedUrl: savedUrl, step3Transport: step3Ok ? 'sendMessage' : '', traceNames, traceRedactedOk, step3Text, saveCallbackOk, saveRouteTraceOk, saveDraftTraceOk, savePayloadCurrentOk, savePatchCallCount: patchCalls.length, savePreviewClosedOk, savePreviewClosedAfterSendOk, saveResultScreenId: saveResult && saveResult.screenId || '', saveVisibleText, repeatSaveIdempotentOk, repeatSaveNewSends, repeatSaveResultScreenId: repeatStaleResult && repeatStaleResult.screenId || '' };
    }



    async function runSaveInFlightVariant() {
      editCalls.length = 0; sendCalls.length = 0; deleteCalls.length = 0; answerCalls.length = 0; patchCalls.length = 0;
      store.setSetupState(USER_ID, { buttonsCurrentCard: target, buttonTargetPost: target, commentTargetPost: target, buttonFlow: null, postEditFlow: null, activeAdminFlowKind: '', buttonsWizardScreenMessageId: '', buttonsWizardScreenOwnerUserId: '', buttonsWizardEditFailedAt: null, buttonsWizardRealShowPathLastDecision: '', buttonSaveRouteTrace: [] });
      await drive(bot, callbackUpdate());
      await drive(bot, textUpdate('Кнопка'));
      const beforeUrlSendCount = sendCalls.length;
      await drive(bot, textUpdate('https://olga.style/inflight'));
      const previewSend = sendCalls.slice(beforeUrlSendCount).find((call) => /Шаг 3\/3/.test(String(call.text || ''))) || null;
      const savePayload = findButtonPayload(previewSend, 'button_admin_save');
      store.setSetupState(USER_ID, { buttonsPendingPreviewSaveInFlightAt: Date.now(), buttonsPendingPreviewSaveInFlightRuntime: 'test-pr215-inflight', buttonsPendingPreviewSaveInFlightToken: 'inflight-token' });
      const beforeSaveEditCount = editCalls.length;
      const beforeSaveSendCount = sendCalls.length;
      const saveResult = savePayload.action ? await drive(bot, callbackUpdateWithPayload(savePayload, previewSend?.mid || previewSend?.messageId || 'preview-msg', 'save-inflight')) : null;
      const inflightFreshSends = sendCalls.slice(beforeSaveSendCount).filter((call) => /Сохранение кнопки уже выполняется/i.test(String(call.text || '')));
      const inflightEdit = editCalls.slice(beforeSaveEditCount).find((call) => call.messageId === (previewSend?.mid || previewSend?.messageId) && /Сохранение кнопки уже выполняется/i.test(String(call.text || '')));
      return { ok: Boolean(saveResult && saveResult.screenId === 'buttons_clean_save_inflight' && inflightEdit && inflightFreshSends.length === 0), screenId: saveResult && saveResult.screenId || '', freshSendCount: inflightFreshSends.length, previewEdited: Boolean(inflightEdit) };
    }
    ...
    const saveCallbackProbeOk = plain.saveCallbackOk && plain.saveRouteTraceOk && plain.repeatSaveIdempotentOk && saveSendFailureFallback.ok && saveInFlight.ok;
    ...
    const realMaxUrlVariantsOk = Object.values(sportsVariants).every((variant) => variant.step1Ok && variant.step2Ok && variant.step3Ok && variant.step3AfterUrl && variant.payload?.screenId === 'buttons_clean_add_preview' && /Предпросмотр кнопки/.test(variant.step3Text || '') !== false);
