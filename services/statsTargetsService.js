...
function channelTarget(c,provider){const id=clean(c.channelId||c.id);return {targetKind:'channel',targetId:id,channelId:id,chatId:'',title:visibleName(c,'Канал без названия'),provider};}
...
function listChannels(userId){const seen=new Set();const out=[];const add=(c,provider,requireStoreBinding)=>{const id=clean(c.channelId||c.id); if(!id||seen.has(id))return; if(requireStoreBinding&&!boundToUser(c,userId,id))return; seen.add(id); out.push(channelTarget(c,provider));}; arr(store.getChannelsList&&store.getChannelsList()).forEach(c=>add(c,'channel_registry',true)); accessChannelsForUser(userId).forEach(c=>add(c,'client_access',false)); return out;}
function listChats(userId, config={}){const providers=[]; const out=[]; const seen=new Set(); const add=(c,provider)=>{const id=clean(c.chatId||c.id||c.requiredChatId); if(!id||seen.has(id))return; if(!boundToUser(c,userId,''))return; seen.add(id); out.push({targetKind:'chat',targetId:id,channelId:'',chatId:id,title:visibleName(c,'Чат без названия'),provider});};
  ...
  for(const [name,fn] of sources){let list=[]; try{list=fn();}catch{} if(list.length)providers.push(name); list.forEach(x=>add(x,name));}
...
function listStatsTargetsForUser(userId, config={}){if(config.statsTargetsOverride){const channels=normalizeOverrideTargets(config.statsTargetsOverride.channels,'channel');const chats=normalizeOverrideTargets(config.statsTargetsOverride.chats,'chat');return {ok:true,userId:clean(userId),channels,chats,targets:[...channels,...chats],diagnostics:{providersUsed:['override'],channelProviderAvailable:true,chatProviderAvailable:chats.length>0,chatProviders:['override']}};} const channels=listChannels(userId); const chat=listChats(userId,config); const channelProviders=[...new Set(channels.map(c=>c.provider).filter(Boolean))]; const providersUsed=channelProviders.concat(chat.providers); return {ok:true,userId:clean(userId),channels,chats:chat.chats,targets:[...channels,...chat.chats],diagnostics:{providersUsed,channelProviderAvailable:channels.length>0,chatProviderAvailable:chat.chatProviderAvailable,chatProviders:chat.providers,pushSubscriptionsUsedAsStatsChats:false}};}
module.exports={listStatsTargetsForUser,_private:{boundToUser,channelPostBoundToUser,accessChannelsForUser}};
