...
function listCampaigns(channelId = '') { const id = clean(channelId); const channels = id ? [channelById(id)] : allChannelsForCampaigns(); return channels.flatMap((ch) => { const settings = getSettings(ch.channelId); return arr(settings.adCampaigns).map((item) => ({ ...item, channelTitle: item.channelTitle || channelTitle(ch), url: normalizeDirectUrl(item.targetUrl || item.url || '') })); }).sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0)); }
