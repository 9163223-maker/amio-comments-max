...
function tenantCtxFor(options = {}) { const userId = clean(options.userId || options.ownerUserId || options.createdByUserId || ''); return userId ? tenantScope.ensureTenantContext(userId) : null; }
function belongsToAdLinkTenant(campaign = {}, ctx = null) {
  if (!ctx) return true;
  const tenantKey = clean(campaign.tenantKey);
  if (tenantKey) return tenantKey === clean(ctx.tenantKey);
  const owners = [campaign.ownerUserId, campaign.createdByUserId, campaign.updatedByUserId, campaign.userId].map(clean).filter(Boolean);
  const allowed = [ctx.ownerUserId, ctx.userId].map(clean).filter(Boolean);
  return Boolean(owners.length && allowed.some((id) => owners.includes(id)));
}
...
function listCampaigns(channelId = '', options = {}) { const id = clean(channelId); const ctx = tenantCtxFor(options); const channels = id ? [channelById(id)] : allChannelsForCampaigns(); return channels.flatMap((ch) => { const settings = getSettings(ch.channelId); return arr(settings.adCampaigns).filter((item) => belongsToAdLinkTenant(item, ctx)).map((item) => ({ ...item, channelTitle: campaignChannelTitle(item, ch), url: normalizeDirectUrl(item.targetUrl || item.url || '') })); }).sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0)); }
function getCampaignBySlug(slug = '', options = {}) { const s = clean(slug).toLowerCase(); return s ? listCampaigns('', options).find((item) => clean(item.slug).toLowerCase() === s) || null : null; }
...
function disableCampaign({ campaignId = '', slug = '', userId = '' } = {}) {
  const ctx = tenantCtxFor({ userId });
  if (!ctx) return { ok: false, reason: 'tenant_required' };
  const wantedId = clean(campaignId);
  const wantedSlug = clean(slug).toLowerCase();
  if (!wantedId && !wantedSlug) return { ok: false, reason: 'campaign_required' };
  for (const ch of allChannelsForCampaigns()) {
    const settings = getSettings(ch.channelId);
    const campaigns = arr(settings.adCampaigns);
    const index = campaigns.findIndex((item) => belongsToAdLinkTenant(item, ctx) && ((wantedId && clean(item.id) === wantedId) || (wantedSlug && clean(item.slug).toLowerCase() === wantedSlug)));
    if (index < 0) continue;
    const updated = { ...campaigns[index], enabled: false, updatedAt: Date.now(), updatedByUserId: clean(ctx.userId) };
    const next = campaigns.slice();
    next[index] = updated;
    store.saveGrowthSettings(ch.channelId, { ...settings, adCampaigns: next });
    return { ok: true, campaign: { ...updated, channelTitle: campaignChannelTitle(updated, ch), url: normalizeDirectUrl(updated.targetUrl || updated.url || '') } };
  }
...
module.exports = { RUNTIME, slugify, listCampaigns, getCampaignBySlug, createCampaign, campaignUrl, trackingUrl, recordCampaignClick, disableCampaign, statsForCampaign, channelTitle, targetUrlForChannel, normalizeDirectUrl, selftest };
