...
function knownUsers() {
  ...
  return unique([
    ...Object.values(n.tenantUsers || {}).map((x) => x.maxUserId),
    ...Object.values(n.tenants || {}).map((x) => x.ownerMaxUserId),
    ...Object.values(n.clients || {}).map((x) => x.maxUserId),
    ...store.getPostsList().map((x) => x.linkedByUserId || x.ownerUserId),
    ...store.getChannelsList().map((x) => x.linkedByUserId || x.ownerUserId)
  ]).slice(0, 50);
}
...
async function buildSelfDiagnostic({ maxUserId = '', label = 'self' } = {}) {
  ...
  const hiddenFromPicker = [...new Set([...tenantIds, ...clientIds])].filter((id) => id && !pickerIds.has(id));
  const violations = [];
  const warnings = [];
  if (!userId) violations.push({ code: 'max_user_id_missing' });
  if (!tenant) violations.push({ code: 'tenant_missing_for_live_user' });
  if (missingBindings.length) violations.push({ code: 'evidence_missing_tenant_binding', channelIdMasked: missingBindings.map(mask) });
  if (hiddenFromPicker.length && tenantChannels.length) violations.push({ code: 'channel_hidden_from_picker', channelIdMasked: hiddenFromPicker.map(mask) });
  ...
  const summary = {
    ...
    active: state.active === true || state.admin === true,
    admin: state.admin === true,
    tenantChannelsCount: tenantChannels.length,
    clientChannelsCount: clientChannels.length,
    pickerChannelsCount: pickerChannels.length,
    storeLinkedChannelsCount: storeLinked.length,
    postEvidenceChannelsCount: unique(postEvidence.map((p) => p.channelId)).length,
    chatExcludedCount: chatsExcluded,
    missingBindingsCount: missingBindings.length,
    hiddenFromPickerCount: hiddenFromPicker.length,
    blockCount: violations.length,
...
function screenLines(d = {}) {
  ...
  if (problems.length) { out.push('', (d.violations || []).length ? 'Что мешает:' : 'Предупреждения:'); problems.slice(0, 6).forEach((x) => out.push(`• ${x.code}`)); }
  else out.push('', 'Серверная связка для этого пользователя выглядит рабочей.');
  out.push('', 'ID показаны сокращённо. Проверяется текущий пользователь, без hardcode.');
  return out;
}
async function buildScreen({ maxUserId = '' } = {}) {
  const d = await buildSelfDiagnostic({ maxUserId, label: 'live_self' });
  return { id: 'account_tenant_diagnostic', text: screenLines(d).join('\n'), attachments: menu.keyboard([[menu.button('Обновить диагностику', 'account_tenant_diagnostic')], [menu.button('Мои каналы', 'account_channels')], [menu.button('Подключить канал', 'admin_section_channels')], [menu.button('Главное меню', 'admin_section_main')]]) };
}
