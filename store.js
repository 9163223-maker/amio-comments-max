...
function sanitizeStoredAttachment(input, stats) {
  ...
  if (source.dataUrl || source.base64 || source.previewData || source.localUrl || source.localOnly || source.uploadError) {
    stats.changed = true;
  ...
  if (!hasStableStoredSource) {
    stats.changed = true;
    stats.removedBrokenAttachments = (stats.removedBrokenAttachments || 0) + 1;
    return null;
  }
  const item = {
    id,
    ...
    storage: String(source.storage || "").slice(0, 60),
      uploadId,
      clientUploadId: String(source.clientUploadId || source.client_upload_id || uploadId || "").slice(0, 180),
      rawUrl,
      processing,
      status,
      transcodeError: String(source.transcodeError || "").slice(0, 220)
  };
  return item;
}
...
function cleanupStoreForRuntime(value) {
  ...
  Object.keys(reactions).forEach((commentKey) => {
    ...
    Object.keys(byComment).forEach((commentId) => {
      ...
      if (!Object.keys(byEmoji).length) {
        delete byComment[commentId];
        stats.changed = true;
      }
    });
    if (!Object.keys(byComment).length) {
      delete reactions[commentKey];
      stats.changed = true;
    }
  });
...
function normalizeStoreShape(parsed) {
  ...
  return {
    ...
    uploadDiagnostics: Array.isArray(parsed.uploadDiagnostics) ? parsed.uploadDiagnostics : [],
    moderation: {
...
function addUploadDiagnostic(entry = {}) {
  if (!Array.isArray(store.uploadDiagnostics)) store.uploadDiagnostics = [];
  const item = {
    id: String(entry.id || (Date.now() + '_' + Math.random().toString(36).slice(2, 8))).slice(0, 120),
    createdAt: Number(entry.createdAt || Date.now()) || Date.now(),
    stage: String(entry.stage || 'unknown').slice(0, 80),
    commentKey: normalizeKey(entry.commentKey || ''),
    type: String(entry.type || '').slice(0, 40),
    fileName: String(entry.fileName || '').slice(0, 180),
    mime: String(entry.mime || '').slice(0, 120),
...
function listUploadDiagnostics({ limit = 100 } = {}) {
  const n = Math.max(1, Math.min(800, Number(limit || 100) || 100));
  return (store.uploadDiagnostics || []).slice(0, n);
}

const RESERVED_DEBUG_KEYS = [
  "ok", "meta", "store", "runtimeVersion", "buildVersion", "displayVersion", "packageVersion",
  "packageName", "sourceMarker", "buildGeneratedAt", "serverStartedAt", "buildInfoSource", "generatedAt", "version"
];

function stripReservedDebugKeys(value) {
  if (!value || typeof value !== "object") return value;
  RESERVED_DEBUG_KEYS.forEach((key) => { if (Object.prototype.hasOwnProperty.call(value, key)) delete value[key]; });
  return value;
}

function getDebugSnapshot() {
  const snapshot = stripReservedDebugKeys(JSON.parse(JSON.stringify(store)));
