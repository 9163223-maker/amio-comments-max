...
const SOURCE = 'adminkit-cc7-5-23-lead-bottom-menu-cleanup';
const MARKER = '__ADMINKIT_CC7_5_23_LEAD_BOTTOM_MENU_CLEANUP_LOADER__';
...
function installRoutes(app) {
  ...
  app.get('/debug/cc7', (req, res) => {
    ...
    res.json({ ok: true, runtimeVersion: RUNTIME, sourceMarker: SOURCE, marker: MARKER, installedAt,
      ...
      adminFlow: (() => { try { return require('./adminkit-admin-flows-7523').selfTest(); } catch (e) { return { ok: false, error: e?.message || String(e) }; } })(),
      ...
      audit: {
        ...
        leadMagnetsCore: 'CC7.5.23 fixes bottom menu cleanup after lead-magnet user input over CC7.5.22',
        ...
        noOverlay: true,
        optimizationNote: 'still layered; after acceptance merge admin-flow wrappers into one clean core module'
      },
      commentOpenStateRoute: require('./routes/commentOpenState').selfTest(), generatedAt: Date.now() });
  });
  app.get(['/debug/ping', '/debug/version', '/debug/build-info'], (req, res) => { noCache(res); res.json({ ok: true, runtimeVersion: RUNTIME, displayVersion: 'CC7.5.23', sourceMarker: SOURCE, buildInfo: readBuildInfo(), loadedLayers, generatedAt: Date.now() }); });
  return app;
}
function installExpressWrap() { if (Module.__adminkitCc7523ExpressWrap) return; Module.__adminkitCc7523ExpressWrap = true; const prev = Module._load; Module._load = function adminkitCc7523Load(request, parent, isMain) { const loaded = prev.apply(this, arguments); if (String(request) === 'express' && loaded && !loaded.__adminkitCc7523Wrapped) { function wrappedExpress(...args) { return installRoutes(loaded(...args)); } Object.setPrototypeOf(wrappedExpress, loaded); Object.assign(wrappedExpress, loaded); wrappedExpress.__adminkitCc7523Wrapped = true; return wrappedExpress; } return loaded; }; }
