...
// CC7.5.17: Gift manager UX over CC7.5.16.
// Goal: apply the same proven algorithm as Buttons to Gifts:
// after post selection detect existing gifts, show manager, allow add/edit/delete, show actual content summary.
// Keeps accepted comments core CC7.5.6 and button manager CC7.5.16 unchanged.
...
const RUNTIME = 'CC7.5.17-GIFT-MANAGER-LIKE-BUTTONS';
const MARKER = '__ADMINKIT_CC7_5_17_GIFT_MANAGER_LIKE_BUTTONS__';
...
async function sendGiftManagerScreen(update, sc, route, opts = {}) {
  ...
  return { handled: true, route, sentKind: 'cc7517_gift_manager_like_buttons', messageId: mid, edited };
}
...
async function giftInputScreen(id, field) {
  await saveS(id, { mode: `cc7517:gifts:${field}` });
  if (field === 'giftTitle') {
...
async function saveGiftToPost(id, f) {
  ...
  if (f.giftEditMode === 'edit') {
    ...
    if (idx === 0) {
      ...
      const j = idx - 1; if (extras[j]) extras[j] = { title, link, requireSubscription: req };
...
async function deleteGiftFromPost(id, commentKey, index) {
  ...
  if (idx === 0) {
    if (extras.length) {
      ...
      await db.query('update ak_post_settings set gifts_enabled=true, gift_title=$3, gift_link=$4, gifts_require_subscription=$5, gifts_json=$6::jsonb, updated_at=now() where admin_id=$1 and comment_key=$2', [id, commentKey, promoted.title, promoted.link, promoted.requireSubscription !== false, JSON.stringify(extras)]);
    } else {
      await db.query("update ak_post_settings set gifts_enabled=false, gift_title='', gift_link='', gifts_json='[]'::jsonb, updated_at=now() where admin_id=$1 and comment_key=$2", [id, commentKey]);
    }
  } else {
...
async function handleAwaitGiftInput(update, id) {
  ...
  if (!mode.startsWith('cc7517:gifts:') && !mode.startsWith('cc7512:gifts:') && !mode.startsWith('cc7511:gifts:') && !mode.startsWith('cc758:gifts:') && !mode.startsWith('cc757:gifts:')) return null;
  ...
  await saveS(id, { mode: '', giftsFlow: flow });
  return { screen: field === 'giftTitle' ? await giftInputScreen(id, 'giftContent') : await giftModeScreen(id), label: field === 'giftTitle' ? 'Название подарка получено' : 'Подарок получен' };
}
...
async function handleGiftClaim(update, p) {
  ...
  return { handled: true, route: 'gifts:claim', sentKind: 'cc7517_gift_sent', giftIndex };
}
...
async function tryHandle(update) {
  ...
  if (awaitedGift) return sendGiftManagerScreen(update, awaitedGift.screen, 'gifts:await', { preferEdit: false, neutralizeLabel: awaitedGift.label });
...
function selfTest() {
  ...
  return { ...b, ok: true, runtimeVersion: RUNTIME, marker: MARKER,
    features: [...(b.features || []), 'gift_manager_like_buttons', 'existing_gift_detection_after_post_select', 'gift_add_edit_delete', 'multi_gifts_json', 'gift_claim_by_index', 'gift_manager_shows_actual_content'],
    commentsCoreTouched: false, buttonsCoreTouched: false, giftsCoreTouched: true,
    policy: 'gift_manager_algorithm_like_buttons_over_7516'
  };
}
