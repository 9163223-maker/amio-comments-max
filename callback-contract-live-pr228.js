...
const LEGACY_LABELS = ['Подписчики', 'Посты', 'Комментарии', 'Реакции', 'Подарки', 'Кнопки под постам', 'Источники подписч'];
...
function failure(errors = []) { return remember({ ok: false, runtimeVersion: process.env.RUNTIME_VERSION || buildInfoRuntime(), sourceMarker: SOURCE, entrypoint: ENTRYPOINT, checkedAt: new Date().toISOString(), mainMenuStatsButtonFound: false, mainMenuStatsPayload: {}, resolvedHandler: '', screenId: '', screenTextPreview: '', renderedRootButtonLabels: [], expectedLabelsPresent: [], legacyLabelsPresent: [], adminSectionStatsRoutesToCurrentStatsRoot: false, adminSectionStatsRoutesToPr226: false, errors: errors.map((e) => clean(e)).filter(Boolean) }); }
...
async function runLiveCallbackContractInProcess() {
  ...
  try {
    ...
    const isPr229RootVariant = /stats_(home|scope_empty|scope_selector|root_menu)_pr229/.test(clean(response.screenId));
    ...
    const adminSectionStatsRoutesToCurrentStatsRoot = legacyLabelsPresent.length === 0 && /stats_(home|scope_empty|scope_selector|root_menu)_pr229|pr226/.test(clean(response.screenId));
    const adminSectionStatsRoutesToPr226 = legacyLabelsPresent.length === 0 && /pr226/.test(clean(response.screenId));
    if (!adminSectionStatsRoutesToCurrentStatsRoot) errors.push('admin_section_stats_not_current_stats_home');
    return { ok: errors.length === 0, runtimeVersion: process.env.RUNTIME_VERSION || buildInfoRuntime() || menu.runtimeVersion(), sourceMarker: SOURCE, entrypoint: ENTRYPOINT, checkedAt: new Date().toISOString(), executionMode: 'child_process_isolated_maxApi' , mainMenuRenderer: 'v3-menu-core-1539.mainScreen', mainMenuStatsButtonFound: Boolean(statsButton), mainMenuStatsPayload, resolvedHandler: clean(response.handler || response.handledBy || 'clean-bot-campaign-attribution-cc8336 -> clean-bot-channel-first-post-picker-pr90'), screenId: clean(response.screenId), screenTextPreview: preview(screenText), renderedRootButtonLabels, expectedLabelsPresent, legacyLabelsPresent, adminSectionStatsRoutesToCurrentStatsRoot, adminSectionStatsRoutesToPr226, errors };
  } catch (error) {
    errors.push(clean(error && error.message || error));
    return { ok: false, runtimeVersion: process.env.RUNTIME_VERSION || buildInfoRuntime(), sourceMarker: SOURCE, entrypoint: ENTRYPOINT, checkedAt: new Date().toISOString(), executionMode: 'child_process_isolated_maxApi', mainMenuStatsButtonFound: false, mainMenuStatsPayload: {}, resolvedHandler: '', screenId: '', screenTextPreview: '', renderedRootButtonLabels: [], expectedLabelsPresent: [], legacyLabelsPresent: [], adminSectionStatsRoutesToCurrentStatsRoot: false, adminSectionStatsRoutesToPr226: false, errors };
  } finally {
