;(() => {
  ...
  async function uploadPacked(packed) {
    ...
    if (flow.timing) {
      flow.timing.uploadStartedAt = now();
      flow.timing.status = 'upload_started';
      flow.timing.uploadSize = Number(packed.size || 0) || 0;
    }
    log('photo_upload_started', timingPayload(flow.timing, { clientUploadId, fileName: packed.fileName, mimeType: packed.mimeType, uploadSize: packed.size, width: packed.width, height: packed.height, quality: packed.quality, maxSide: packed.maxSide, hasDataUrl: true, status: 'upload_started', durationMs: 0 }));
    ...
    if (flow.timing) flow.timing.uploadEndedAt = now();
    if (!response.ok || data.ok === false || !data.attachment) {
      log('photo_upload_failed', timingPayload(flow.timing, { clientUploadId, status: 'upload_failed', durationMs: flow.timing ? flow.timing.uploadEndedAt - flow.timing.uploadStartedAt : 0, error: clean(data.error || data.message || 'photo_upload_failed') }));
    ...
    if (flow.timing) {
      flow.timing.uploadId = attachment.uploadId;
      flow.timing.status = 'upload_ok';
    }
    log('photo_upload_ok', timingPayload(flow.timing, { clientUploadId, uploadId: attachment.uploadId, fileName: attachment.fileName, mimeType: attachment.mimeType, uploadSize: attachment.size || packed.size, hasUrl: Boolean(attachment.url), hasPreviewUrl: Boolean(attachment.previewUrl), status: 'upload_ok', durationMs: flow.timing ? flow.timing.uploadEndedAt - flow.timing.uploadStartedAt : 0 }));
    return attachment;
  }
  function apiRender() {
  ...
  async function sendPreview() {
    ...
    if (flow.timing) {
      flow.timing.sendClickedAt = now();
      flow.timing.status = 'send_clicked';
    }
    ...
    try {
      ...
      if (!packed) {
        if (flow.timing) flow.timing.compressStartedAt = now();
        packed = await compressImage(currentFile);
        if (flow.timing) {
          flow.timing.compressEndedAt = now();
          flow.timing.compressedSize = Number(packed.size || 0) || 0;
          flow.timing.uploadSize = Number(packed.size || 0) || 0;
          flow.timing.width = Number(packed.width || 0) || 0;
          flow.timing.height = Number(packed.height || 0) || 0;
          flow.timing.quality = Number(packed.quality || 0) || 0;
          flow.timing.maxSide = Number(packed.maxSide || 0) || 0;
        }
        if (flow.selectionToken !== selectionToken || flow.file !== currentFile) {
          log('photo_send_stale_selection_blocked', timingPayload(flow.timing, { selectionToken, status: 'send_stale_selection_blocked' }));
      ...
      if (flow.selectionToken !== selectionToken || flow.file !== currentFile) {
        log('photo_upload_stale_selection_blocked', timingPayload(flow.timing, { selectionToken, currentSelectionToken: flow.selectionToken, status: 'upload_stale_selection_blocked' }));
      ...
      if (flow.timing) {
        flow.timing.totalEndedAt = now();
        flow.timing.status = 'ok';
      }
      log('photo_timing_summary', timingPayload(flow.timing, { status: timingStatus(flow.timing, 'ok'), durationMs: flow.timing ? flow.timing.totalEndedAt - flow.timing.startedAt : 0 }));
      ...
      if (flow.timing) {
        flow.timing.totalEndedAt = now();
        flow.timing.status = 'failed';
        flow.timing.error = message;
      }
      log('photo_timing_summary', timingPayload(flow.timing, { status: timingStatus(flow.timing, 'failed'), error: message, durationMs: flow.timing ? flow.timing.totalEndedAt - flow.timing.startedAt : 0 }));
  ...
  async function handleFile(fileInput) {
    ...
    log('photo_selected', timingPayload(flow.timing, { fileName: clean(file.name), mimeType: mime, originalSize: Number(file.size || 0) || 0, selectionToken, status: 'selected', durationMs: 0 }));
    renderPreview(file, flow.previewUrl);
    try {
      const started = now();
      if (flow.timing) flow.timing.compressStartedAt = started;
      ...
      if (flow.timing) {
        ...
        flow.timing.quality = Number(packed.quality || 0) || 0;
        flow.timing.maxSide = Number(packed.maxSide || 0) || 0;
        flow.timing.status = 'compressed';
      }
      if (flow.selectionToken !== selectionToken || flow.file !== file) {
        log('photo_compress_stale_ignored', timingPayload(flow.timing, { fileName: clean(file.name), selectionToken, currentSelectionToken: flow.selectionToken, status: 'compress_stale_ignored' }));
        return;
      }
      flow.packed = packed;
      flow.packedToken = selectionToken;
      previewStatus('', false);
      log('photo_compress_ok', timingPayload(flow.timing, { fileName: packed.fileName, mimeType: packed.mimeType, compressedSize: packed.size, uploadSize: packed.size, width: packed.width, height: packed.height, quality: packed.quality, maxSide: packed.maxSide, durationMs: flow.timing ? flow.timing.compressEndedAt - started : now() - started, selectionToken, status: 'compress_ok' }));
    } catch (error) {
      if (flow.selectionToken !== selectionToken || flow.file !== file) {
        log('photo_compress_stale_error_ignored', timingPayload(flow.timing, { fileName: clean(file.name), selectionToken, error: clean(error && error.message), status: 'compress_stale_error_ignored' }));
        return;
      }
      if (flow.timing) {
        flow.timing.compressEndedAt = now();
        flow.timing.status = 'compress_failed';
        flow.timing.error = clean(error && error.message);
      }
      previewStatus('Не удалось обработать фото. Попробуйте другое изображение.', true);
      log('photo_compress_failed', timingPayload(flow.timing, { fileName: clean(file.name), error: clean(error && error.message), selectionToken, status: 'compress_failed', durationMs: flow.timing ? flow.timing.compressEndedAt - flow.timing.compressStartedAt : 0 }));
    }
  }
