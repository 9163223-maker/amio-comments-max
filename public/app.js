...
const PERFORMANCE_TRACE_RUNTIME = 'CC8.1.14-PERFORMANCE-TRACE-PR73';
...
const ASSET_VERSION = 'v7564-pr73';
...
function postMiniTiming(name, extra) {
  try {
    const payload = {
      ...
      durationMs: Date.now() - LOADER_STARTED_AT,
      sinceLoaderStartMs: Date.now() - LOADER_STARTED_AT,
      navStartMs: performance && performance.timeOrigin ? Math.round(LOADER_STARTED_AT - performance.timeOrigin) : 0,
      ...(extra || {})
    };
    const body = JSON.stringify(payload);
    if (navigator && typeof navigator.sendBeacon === 'function') {
      const ok = navigator.sendBeacon('/api/debug/miniapp-timing', new Blob([body], { type: 'application/json' }));
      if (ok) return;
    }
