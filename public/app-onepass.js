...
function selectMediaSource(attachment) {
  ...
  const broken = Boolean(att.brokenRuntimeOnly || att.runtimeOnlyBroken || (runtimeOnly && att.runtimeOnly));
...
function postMediaCandidates(source) {
  ...
  function addCandidate(att, defaultName) {
    ...
    if (isRuntimeCommentUploadUrl(selected.url) && !isInlinePreviewSource(selected.url)) return;
