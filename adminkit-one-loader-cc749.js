...
const SOURCE = 'adminkit-cc7-4-9-archive-direct-menu';
const MARKER = '__ADMINKIT_CC7_4_9_ARCHIVE_DIRECT_MENU_LOADER__';
...
function loadLayer(pathName) {
  ...
  try {
    ...
    console.warn('[cc7.4.9] layer failed:', pathName, item.error);
  }
...
function installAdminFlowPatch() {
  try {
    ...
    if (postPatcher.__adminkitCc749Patched || postPatcher.__adminkitCc748Patched || postPatcher.__adminkitCc747Patched) {
    ...
    postPatcher.patchStoredPost = async function adminkitCc749PatchStoredPost(args = {}) {
      if (args && args.commentKey) markDirty(args.commentKey, 'cc749_before_patchStoredPost');
      return originalPatchStoredPost.call(this, args);
    };
    postPatcher.__adminkitCc749Patched = true;
    adminFlowPatch = { ok: true, runtimeVersion: RUNTIME, patched: ['postPatcher.patchStoredPost'], policy: 'saved_buttons_and_gifts_force_repatch_and_persist_customKeyboard_in_post_snapshot' };
    return adminFlowPatch;
  } catch (error) {
    adminFlowPatch = { ok: false, error: error?.message || String(error) };
    console.warn('[cc7.4.9] admin flow patch failed:', adminFlowPatch.error);
    return adminFlowPatch;
  }
}
...
function installRoutes(app) {
  if (!app || app.__adminkitCc749Routes) return app;
  app.__adminkitCc749Routes = true;
  ...
  app.get('/debug/cc7', (req, res) => {
    ...
    res.json({ ok: true, runtimeVersion: RUNTIME, sourceMarker: SOURCE, marker: MARKER, installedAt, policy: 'archive_direct_menu_plus_admin_saved_buttons_and_gifts_force_repatch', appOnepass: appOnepassInfo(), adminFlowPatch, postPatcherRuntime, useOpenAppButton: process.env.ADMINKIT_USE_OPEN_APP_BUTTON, buildInfo: readBuildInfo(), loadedLayers, commentOpenStateRoute: require('./routes/commentOpenState').selfTest(), generatedAt: Date.now() });
  });
  app.get(['/debug/ping', '/debug/version', '/debug/build-info'], (req, res) => {
    noCache(res);
    res.json({ ok: true, service: 'amio-comments-max', runtimeVersion: RUNTIME, buildVersion: RUNTIME, displayVersion: 'CC7.4.9', sourceMarker: SOURCE, useOpenAppButton: process.env.ADMINKIT_USE_OPEN_APP_BUTTON, adminFlowPatch, buildInfo: readBuildInfo(), generatedAt: Date.now(), installedAt, commentOpenStateRoute: require('./routes/commentOpenState').selfTest() });
  });
  return app;
}

function installExpressWrap() {
  if (Module.__adminkitCc749ExpressWrap) return;
  Module.__adminkitCc749ExpressWrap = true;
  const prev = Module._load;
  Module._load = function adminkitCc749Load(request, parent, isMain) {
    const loaded = prev.apply(this, arguments);
    try {
      if (String(request) === 'express' && loaded && !loaded.__adminkitCc749Wrapped) {
        function wrappedExpress(...args) { return installRoutes(loaded(...args)); }
        Object.setPrototypeOf(wrappedExpress, loaded);
        Object.assign(wrappedExpress, loaded);
        wrappedExpress.__adminkitCc749Wrapped = true;
        return wrappedExpress;
      }
    } catch (error) { console.warn('[cc7.4.9] express wrap failed:', error?.message || error); }
    return loaded;
  };
}
