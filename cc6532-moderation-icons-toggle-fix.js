...
// CC6.5.3.2 moderation icons + reliable toggle fix.
// Fixes user issues from moderation post screen:
// - add icons to make actions clearer;
// - keep both channel and scope visible;
// - no ellipsis in button labels;
// - links toggle changes status and the next button label in the same edited message;
// - "Проверить комментарий" is a working route;
// - includes stress test with explicit link-toggle state transition.

const Module = require('module');

const RUNTIME = 'CC6.5.3.2';
const SOURCE = 'adminkit-CC6.5.3.2-moderation-icons-toggle-fix';
const events = [];
...
const ROUTES_TO_STRESS = [
  ...
  'moderation:test_comment_example',
  'help:moderation'
];
...
function canonical(action = '') {
  ...
  const aliases = {
    ...
    'moderation:post_rules': 'moderation:choose_post', 'moderation:rules_post': 'moderation:choose_post',
    'moderation:enable_filter': 'moderation:toggle_filter', 'moderation:disable_filter': 'moderation:toggle_filter',
    'moderation:basic_words': 'moderation:base_words',
    'moderation:links_allow': 'moderation:toggle_links', 'moderation:links_block': 'moderation:toggle_links',
    'moderation:allow_links': 'moderation:toggle_links', 'moderation:block_links': 'moderation:toggle_links',
    'moderation:invites_allow': 'moderation:toggle_invites', 'moderation:invites_block': 'moderation:toggle_invites',
    'moderation:ai_enable': 'moderation:toggle_ai', 'moderation:ai_disable': 'moderation:toggle_ai'
  };
...
function setModeration(extra = {}, patch = {}) { const storeModule = storeObj(); const key = scopeKey(extra); const next = { ...getModeration(extra), ...patch, updatedAt: Date.now() }; storeModule.store.moderation.byChannel[key] = next; saveStore(); return next; }
...
function actionFilter(value) { return value ? '⏸ Фильтр' : '✅ Фильтр'; }
function actionBase(value) { return value ? '🧱 База выкл' : '🧱 База вкл'; }
function actionLinks(value) { return value ? '🔗 Запретить' : '🔗 Разрешить'; }
function actionInvites(value) { return value ? '✉️ Запретить' : '✉️ Разрешить'; }
...
function moderationText(extra = {}) {
  ...
  return [
    ...
    '',
    `🛡 Фильтр: ${onOff(state.filter)} ${yesNo(state.filter)} · 🧱 База: ${onOffPlural(state.basicWords)} ${yesNo(state.basicWords)}`,
    `➕ Стоп-слова: ${(state.manualWords || []).length} · 🔗 Ссылки: ${allowText(state.links)} ${yesNo(state.links)}`,
    `✉️ Инвайты: ${allowText(state.invites)} ${yesNo(state.invites)} · 🤖 AI: ${onOff(state.ai)} ${yesNo(state.ai)}`
  ].filter(Boolean).join('\n');
}
...
function moderationRows(extra = {}) {
  ...
  return [
    [btn('🛡 Канал', 'moderation:channel_rules', ch), btn('🎯 Пост', 'moderation:choose_post', ch)],
...
function choosePostModel(extra = {}) {
  ...
  return {
    text: ['🛡 Модерация → выбор поста', `📺 Канал: ${channel ? channelTitle(channel) : 'не выбран'}`, '', `Постов найдено: ${posts.length}`, 'Выберите пост для отдельных правил.'].join('\n'),
    attachments: kb([...posts.map((post, index) => [btn(postTitle(post, index), 'moderation:post', { channelId, commentKey: postKey(post), postId: norm(post.postId) })]), ...nav(channelOnly(extra))])
  };
}
...
function baseWordsModel(extra = {}) {
  ...
  return {
    text: ['🧱 Базовые стоп-слова', `Статус: ${onOffPlural(state.basicWords)} ${yesNo(state.basicWords)}`, `Всего слов и фраз: ${BASE_STOP_WORDS.length}`, '', `Пример: ${preview}${hidden ? ` и ещё ${hidden}` : ''}.`, '', 'Список закрывает массовый спам, мошенничество, ставки, агрессивные приглашения, подозрительные заработки и запрещённые темы.'].join('\n'),
    attachments: kb([[btn(actionBase(state.basicWords), 'moderation:toggle_basic_words', extra)], [btn('➕ Стоп-слово', 'moderation:manual_words', extra)], ...nav(extra)])
  };
}
...
function manualWordsModel(extra = {}) {
  ...
  return {
    text: ['➕ Ручные стоп-слова', words.length ? `Список: ${words.slice(-12).join(', ')}` : 'Список пуст.', '', 'Здесь администратор добавляет свои слова и фразы. Для теста кнопка добавляет слово «спам».'].join('\n'),
    attachments: kb([[btn('➕ Добавить стоп-слово', 'moderation:add_word', extra)], [btn('➖ Удалить последнее', 'moderation:remove_word', extra), btn('🧹 Очистить список', 'moderation:clear_manual_words', extra)], ...nav(extra)])
  };
}

function statusActionModel(route, extra = {}) {
  if (route === 'moderation:logs') {
    return { text: ['📋 Журнал модерации', '', 'Пока журнал пуст.', 'Здесь будут последние срабатывания фильтра: слово, причина, пост и действие администратора.'].join('\n'), attachments: kb(nav(extra)) };
  }
  return { text: ['🧪 Проверить комментарий', '', 'Отправьте текст комментария следующим сообщением.', 'Бот покажет результат проверки: пропустить, скрыть или отправить на ручную модерацию.'].join('\n'), attachments: kb([[btn('🧪 Пример проверки', 'moderation:test_comment_example', extra)], ...nav(extra)]) };
}

function testCommentExample(extra = {}) {
  const state = getModeration(extra);
  const blockedByWords = state.filter && state.basicWords;
  const blockedByLinks = state.filter && !state.links;
  const decision = blockedByWords || blockedByLinks ? 'скрыть' : 'пропустить';
  const reason = blockedByWords ? 'найдено базовое стоп-слово' : (blockedByLinks ? 'ссылки запрещены настройками' : 'нарушений не найдено');
  return { text: ['🧪 Проверить комментарий', '', 'Тестовый текст: «спам, перейди по ссылке»', `Результат: ${decision}`, `Причина: ${reason}.`].join('\n'), attachments: kb(nav(extra)) };
}

function helpModel(extra = {}) {
  return { text: ['❓ Помощь: Модерация', '', 'Одна настройка — одна кнопка. Нажатие меняет состояние и сразу обновляет верхнюю сводку.', '', 'Канал/пост всегда видны сверху. Базовые слова и ручные стоп-слова открываются отдельными экранами.'].join('\n'), attachments: kb(nav(extra)) };
}
...
async function model(routeRaw = 'moderation:home', extra = {}, mutate = true) {
  ...
  if (route === 'moderation:toggle_basic_words') { if (mutate) toggleModeration('basicWords', extra); return moderationHome(extra); }
...
async function stress() {
  ...
  for (const route of ROUTES_TO_STRESS) {
    try {
      const rendered = await model(route, { channelId: '__stress_channel__', commentKey: route === 'moderation:post' ? '__stress_post__' : '' }, false);
      const labels = (rendered?.attachments?.[0]?.payload?.buttons || []).flat().map((b) => b.text || '');
      results.push({ route, ok: Boolean(rendered && rendered.text && rendered.attachments), buttons: labels.length, hasEllipsisInButton: labels.some((label) => label.includes('…') || label.includes('...')), labels });
    } catch (error) {
      results.push({ route, ok: false, error: error?.message || String(error) });
    }
  }
  const toggleExtra = { channelId: '__stress_channel__', commentKey: '__stress_post_toggle_links__' };
  setModeration(toggleExtra, { links: true });
  const before = getModeration(toggleExtra).links;
  await model('moderation:toggle_links', toggleExtra, true);
  const after = getModeration(toggleExtra).links;
  const afterScreen = await model('moderation:post', toggleExtra, false);
  const afterLabels = (afterScreen?.attachments?.[0]?.payload?.buttons || []).flat().map((b) => b.text || '');
  const toggleLinksOk = before === true && after === false && /Ссылки: запрещены/.test(afterScreen.text) && afterLabels.includes('🔗 Разрешить');
  return {
    ok: results.every((item) => item.ok && !item.hasEllipsisInButton) && toggleLinksOk,
...
async function runtimeCheck(res) {
  ...
  return sendText(res, [
    'OK: ' + (s.ok ? 'MODERATION_ICONS_TOGGLE_STRESS_PASS' : 'MODERATION_ICONS_TOGGLE_STRESS_FAIL'),
    'runtime: ' + RUNTIME,
    'sourceMarker: ' + SOURCE,
    'routesTotal: ' + s.total,
    'routesPassed: ' + s.passed,
    'toggleLinksPostScope: ' + (s.toggleLinksOk ? 'pass' : 'fail'),
    'iconsInButtons: enabled',
    'scopeHeader: channel_and_area_visible',
...
function installExpressPatch() {
  if (Module._load.__cc6532Patch) return;
  const oldLoad = Module._load;
  function patchedLoad(request, parent, isMain) {
    const loaded = oldLoad.apply(this, arguments);
    if (String(request || '') === 'express' && loaded && !loaded.__cc6532Wrap) {
      function expressWrapper() {
        ...
        if (app && !app.__cc6532) {
          ...
          app.use((req, res, next) => {
            ...
            if (route === '/debug/moderation-icons-toggle') return runtimeCheck(res);
            if (route === '/debug/moderation-icons-toggle-json') { noCache(res); return stress().then((s) => res.json({ ok: s.ok, runtimeVersion: RUNTIME, sourceMarker: SOURCE, stress: s })); }
            if (route === '/debug/moderation-icons-toggle-events') { noCache(res); return res.json({ ok: true, runtimeVersion: RUNTIME, sourceMarker: SOURCE, events: events.slice(-120) }); }
            return next();
          });
      ...
      expressWrapper.__cc6532Wrap = true;
      return expressWrapper;
    }
    return loaded;
  }
  patchedLoad.__cc6532Patch = true;
