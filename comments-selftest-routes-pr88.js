...
function runnerHtml() {
  return '<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,viewport-fit=cover"><title>AdminKit PR97 comments matrix selftest</title><link rel="stylesheet" href="/comments-selftest-runner-pr89.css"><main><h1>AdminKit PR97 production comments matrix</h1><section class="card"><div id="status"><span class="pill warn">Готов к запуску</span></div><p class="muted">Одна страница запускает backend self-test, открывает настоящий comments UI в iframe, проверяет text/photo/reply/reaction/sticker/forbidden-file/original-media/hydration/trace matrix и отправляет mandatory production_comments_matrix_probe. Видео/файлы проверяются только на отказ.</p><div class="actions"><button id="runBtn" type="button">Run PR97 matrix self-test</button><a id="latestLink" class="button secondary" href="/debug/selftest/comments/latest">Latest JSON</a><a id="reportLink" class="button secondary" href="/debug/selftest/comments/report">HTML report</a><a id="cleanupLink" class="button danger" href="#" hidden>Очистить fixtures</a></div></section><section class="card"><h2>Итог</h2><div id="summary">Пока не запускали.</div></section><section class="card"><h2>Browser fixture</h2><div id="commentsFixture"><div id="commentsList"></div></div></section><section class="card"><h2>Шаги</h2><div id="log" class="log"></div></section><section class="card"><h2>Raw final report</h2><pre id="raw">{}</pre></section></main>' + runnerPrebootPatch() + '<script src="/comments-selftest-runner-pr89.js"></script>';
}
