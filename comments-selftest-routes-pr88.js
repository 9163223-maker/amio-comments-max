...
const { runFullCommentsSelftest, getLatestReport, RUNTIME } = require('./services/commentsSelftestPr88');
...
function install(app) {
  ...
  app.get('/debug/selftest/comments/report', (req, res) => {
    ...
    const rows = (report.tests || []).map((item) => `<tr><td>${item.status === 'pass' ? '✅' : '❌'}</td><td>${item.id}</td><td>${item.expected || ''}</td><td><pre>${JSON.stringify(item.actual || item.details || {}, null, 2)}</pre></td></tr>`).join('');
    return res.type('html').send(`<!doctype html><meta charset="utf-8"><title>AdminKit comments selftest</title><style>body{font-family:system-ui;margin:24px;background:#f7fafc;color:#102030}table{border-collapse:collapse;width:100%;background:white}td,th{border:1px solid #d8e2ea;padding:8px;vertical-align:top}pre{white-space:pre-wrap;max-width:640px} .ok{color:#137333}.bad{color:#b3261e}</style><h1>AdminKit comments selftest</h1><p>Runtime: <b>${report.runtimeVersion || RUNTIME}</b></p><p class="${report.ok ? 'ok' : 'bad'}">Status: ${report.ok ? 'PASS' : 'FAIL'}</p><p>Passed: ${report.summary?.passed || 0}; Failed: ${report.summary?.failed || 0}; Total: ${report.summary?.total || 0}</p><p>CommentKey: <code>${report.commentKey || ''}</code></p><table><thead><tr><th>Status</th><th>Test</th><th>Expected</th><th>Details</th></tr></thead><tbody>${rows}</tbody></table>`);
  });
