...
function postByKey(key){try{return store.getPost(key)||null;}catch{return null;}}
...
function questionScreen(menu,commentKey,question=''){
...
async function customStart(menu,{userId='',commentKey=''}={}){
  if(!postByKey(commentKey)) return {id:'poll_error',text:['⚠️ Пост не найден','','Нужно выбрать пост из сохранённых.'].join('\n'),attachments:menu.keyboard([[menu.button('📌 Выбрать пост','comments_select_post',{source:'polls'})],[menu.button('🏠 Главное меню','admin_section_main')]])};
  await setFlow(userId,{type:'poll_custom',step:'question',commentKey,startedAt:Date.now()});
  return questionScreen(menu,commentKey);
}
async function customCancel(menu,{userId='',commentKey=''}={}){await clearFlow(userId);return base.picked(menu,commentKey);}
async function customEditQuestion(menu,{userId='',commentKey=''}={}){
  ...
  return questionScreen(menu,key,flow.question||'');
}
