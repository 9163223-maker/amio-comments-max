...
/**
 * CC5.5 moderation wrapper.
 * Purpose: remove legacy escape routes from the moderation section.
 * - Main menu callback is handled inside CC router, not by legacy.
 * - Channel titles are repaired from MAX before menus are rendered.
 * - Self-test checks routing that matches real user paths, not just flags.
 */
...
function mainMenuPacket(displayName = '') {
  ...
  return {
    text: ['🐋 АдминКИТ', '', `${hello}`, 'АдминКИТ — панель управления MAX-каналом.', 'Выберите раздел: комментарии, модерация, редактор, кнопки, подарки или статистика.'].join('\n'),
    attachments: keyboard([
      [button('💬 Комментарии', 'comments_menu')],
      [button('🛡 Модерация', 'mod_start')],
      [button('✏️ Редактор постов', 'editor_menu')],
      [button('⚪ Кнопки под постами', 'buttons_menu')],
      [button('🎁 Подарки / лид-магниты', 'gifts_menu')],
      [button('📊 Статистика', 'stats_menu')],
      [button('📺 Каналы и подключение', 'channels_menu')],
      [button('❓ Помощь', 'help_menu')]
    ])
  };
}
async function sendOrEdit(update, currentAdminId, packet) {
  ...
  if (mid) {
    try {
      ...
      console.warn('[CC5.5 main menu edit]', error && error.message ? error.message : error);
    }
  }
...
async function repairChannelTitle(channelId) {
  ...
  try {
    ...
    console.warn('[CC5.5 channel title repair]', channelId, error && error.message ? error.message : error);
  }
  return null;
}
async function repairKnownChannelTitles(currentAdminId, explicitChannelId = '') {
  ...
  for (const ch of channels.slice(0, 10)) {
    ...
    if (title && title !== oldTitle) {
      await db.upsertChannel(currentAdminId, id, title, { source: 'cc55_title_repair' });
      updated += 1;
    }
  }
...
async function handle(update = {}) {
  ...
  if (action(update).startsWith('mod') || action(update) === 'модерация' || channelId) {
...
function selfTest() {
  ...
  return { ok: Object.values(checks).every(Boolean), runtime: RUNTIME, sourceMarker: SOURCE, checks, base: baseTest };
}
