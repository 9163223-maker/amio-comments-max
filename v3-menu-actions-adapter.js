...
// Replaces empty/stub V3 screens with concrete route handlers.
// It does not change comment open_app buttons and does not repatch old posts unless admin clicks a post-scoped save/toggle action.

const RUNTIME = 'CC6.6.9-V3-MENU-ACTIONS-ADAPTER';
const SOURCE = 'adminkit-v3-every-menu-button-has-action-v1';
...
const SECTIONS = {
  ...
  polls: ['🗳 Опросы', 'Голосования и опросы. Раздел в разработке, но дерево действий закреплено.'],
...
const CHILDREN = {
  ...
  polls: [['polls:create','➕ Создать'],['polls:attach_post','📌 Привязать к посту'],['polls:options','✏️ Варианты'],['polls:toggle_multiple','✅ Один/несколько'],['polls:results','📊 Результаты'],['polls:finish','⏸ Завершить'],['polls:preview','👀 Предпросмотр']],
...
async function posts(adminId, channelId, limit = 30) {
  ...
  try { return store().getPostsList().filter((p) => !channelId || String(p.channelId) === String(channelId)).slice(0, limit).map((p) => ({ postId: p.postId, commentKey: p.commentKey, title: p.title || p.originalText || p.postId, messageId: p.messageId, channelId: p.channelId })); } catch { return []; }
}
async function active(adminId, payload = {}) {
  const cs = await channels(adminId); const c = norm(payload.c || payload.channelId || cs[0]?.channelId || '');
  const ps = await posts(adminId, c, 50); const p = ps.find((x) => norm(x.postId) === norm(payload.p) || norm(x.commentKey) === norm(payload.k)) || ps[0] || null;
  return { channels: cs, channelId: c, channelTitle: norm(cs.find((x) => norm(x.channelId) === c)?.title || c || 'Канал не выбран'), posts: ps, post: p, commentKey: norm(payload.k || p?.commentKey || '') };
}
...
async function home(route) { const o = owner(route); const s = SECTIONS[o] || ['АдминКИТ','Раздел']; return screen([s[0], '', s[1], '', 'Выберите действие.'], [...rows2((CHILDREN[o] || []).map(([r,t]) => btn(t,r))), ...nav(o)]); }

async function pickPost(adminId, route, payload) {
  const o = owner(route); const a = await active(adminId, payload); const target = o === 'editor' ? 'editor:post' : o === 'buttons' ? 'buttons:post' : o === 'gifts' ? 'gifts:post' : o === 'moderation' ? 'moderation:post' : o === 'highlight' ? 'highlight:post' : o === 'stats' ? 'stats:post' : 'comments:post';
  const rows = a.posts.slice(0, 10).map((p,i) => [btn(`${i+1}. ${short(p.title || p.postId, 42)}`, target, { c:a.channelId, p:p.postId, k:p.commentKey })]);
  return screen([SECTIONS[o]?.[0] || 'Посты', '', `Канал: ${a.channelTitle}`, `Постов в памяти: ${a.posts.length}`, '', a.posts.length ? 'Выберите пост.' : 'Постов нет в памяти. Перешлите опубликованный пост боту — он зарегистрирует пост и сохранит текст для комментариев.'], [...rows, ...nav(o)]);
}

async function postCard(adminId, route, payload) {
  const o = owner(route); const a = await active(adminId, payload); const p = a.post || {}; const base = { c:a.channelId, p:p.postId || payload.p || '', k:a.commentKey || payload.k || '' };
  const byOwner = {
...
async function channelsRoute(adminId, route, payload) {
  ...
  if (route === 'channels:list' || route === 'channels:select') {
    const rows = cs.slice(0, 10).map((c,i) => [btn(`${i+1}. ${short(c.title || c.channelId, 45)}`, 'channels:home', { c:c.channelId })]);
  ...
  if (route === 'channels:verify_access') return screen(['✅ Проверить права бота', '', `Активный канал: ${a.channelTitle}`, 'Проверка подготовлена: бот должен быть администратором канала и иметь право редактировать сообщения, чтобы добавлять комментарии/кнопки.', '', 'Live-проверка прав будет выполняться через MAX API без долгого ожидания меню.'], [[btn('📋 Мои каналы','channels:list'), btn('➕ Подключить','channels:connect')], ...nav('channels')]);
  return screen(['🔐 Доступы канала', '', `Активный канал: ${a.channelTitle}`, 'Функция показывает, какие возможности доступны каналу: комментарии, фото, кнопки, подарки, статистика.', '', route === 'channels:admins' ? 'Администраторы канала — бизнес-раздел в разработке.' : 'Базовая проверка доступа активна.'], [[btn('✅ Проверить права','channels:verify_access')], ...nav('channels')]);
}

async function commentsRoute(adminId, route, payload) {
  const a = await active(adminId, payload);
  if (route === 'comments:auto_new') { try { if (a.channelId) store().saveChannel(a.channelId, { autoModeEnabled: true, autoCommentsV3: true }); } catch {} return screen(['⚡ Авто для новых постов', '', `Канал: ${a.channelTitle}`, 'Функция активна: новые посты, попавшие в webhook/память бота, получают связь с обсуждением. Старые посты не переписываются автоматически.', '', 'Для уже опубликованного поста используйте «Старый пост».'], [[btn('📌 Старый пост','comments:old_post'), btn('📌 Выбрать пост','comments:choose_post',{c:a.channelId})], ...nav('comments')]); }
  if (route === 'comments:old_post') { await setFlow(adminId, { type:'comments_old_post', step:'await_forwarded_post' }); return screen(['📌 Старый пост', '', 'Действие активно.', 'Перешлите сюда уже опубликованный пост из канала. Бот зарегистрирует его, сохранит текст поста и безопасно добавит/восстановит кнопку комментариев.', '', 'Повторный патч должен быть идемпотентным: без дублей и без потери текста.'], [[btn('📌 Выбрать пост','comments:choose_post',{c:a.channelId})], ...nav('comments')]); }
  if (route === 'comments:preview') return screen(['👀 Как это выглядит', '', 'Открывается мини-приложение обсуждения в Telegram-like стиле MAX: верхняя навигация, счётчик комментариев, поле ввода, реакции и ответы.', '', 'Если вверху видно «Загрузка…», fallback CC6.6.8 восстанавливает текст поста из базы/live-сообщения.'], [[btn('📌 Выбрать пост','comments:choose_post',{c:a.channelId})], ...nav('comments')]);
  ...
  if (route === 'comments:toggle') return screen(['✅/⏸ Комментарии', '', `Пост: ${short(a.post?.title || a.post?.postId || 'не выбран')}`, 'Функция привязана к postEditorService.setPostCommentsEnabled. Чтобы не сломать старые посты случайным нажатием, переключение выполняется только после выбора конкретного поста и подтверждения в карточке поста.', '', 'Сейчас используйте «Выбрать пост».'], [[btn('📌 Выбрать пост','comments:choose_post',{c:a.channelId})], ...nav('comments')]);
  return home('comments:home');
}
...
async function moderationRoute(adminId, route, payload) {
  ...
  const lines = ['🛡 Модерация', '', `Канал: ${a.channelTitle}`, `Фильтр: ${settings?.enabled === false ? 'выключен' : 'включен'}`, `Ссылки: ${settings?.blockLinks ? 'запрещены' : 'разрешены'}`, `Инвайты: ${settings?.blockInvites === false ? 'разрешены' : 'запрещены'}`, '', route === 'moderation:add_word' ? 'Пришлите следующим сообщением стоп-слово для добавления.' : route === 'moderation:test_comment' ? 'Пришлите следующим сообщением тестовый комментарий для проверки.' : 'Выберите действие.'];
  return screen(lines, [...rows2((CHILDREN.moderation || []).map(([r,t]) => btn(t,r,{c:a.channelId}))), ...nav('moderation')]);
}
...
async function flowRoute(adminId, route, payload) {
  ...
  const dev = ['highlight','polls','billing','referrals'].includes(o);
  const text = {
    editor: 'Функция редактора активна: выберите пост, затем отправьте новый текст. Медиа, ссылки и кнопки должны сохраняться через postEditorService.',
    buttons: 'Функция кнопок активна как 3 шага: 1) пост, 2) текст кнопки, 3) ссылка/действие, затем сохранить.',
    gifts: 'Функция подарков активна как 4 шага: 1) канал и пост, 2) файл или ссылка, 3) сообщение получателю, 4) подтверждение.',
    highlight: 'Раздел выделения постов в разработке. Маршрут закреплён, выбранный пост сохраняется в flow.',
    polls: 'Раздел опросов в разработке. Маршрут закреплён, сценарий создания опроса сохранён в flow.',
    billing: 'Платёжный модуль в разработке. Маршрут закреплён, тарифные действия не смешиваются с комментариями.',
    referrals: 'Реферальный модуль в разработке. Маршрут закреплён.'
  }[o] || 'Действие запущено.';
...
async function simpleRoute(adminId, route, payload) {
  const o = owner(route); const a = await active(adminId, payload);
  ...
  if (o === 'comments_banner') return screen(['🖼 Баннер в обсуждениях', '', 'Функция привязана к обсуждению, не к CTA-кнопке под постом.', 'Сейчас доступен предпросмотр и маршруты настройки. Точный визуальный редактор баннера — в разработке.'], [[btn('💬 В комментарии','comments:home')], ...nav('comments')]);
  if (o === 'comments_photo') return screen(['📷 Фото в комментариях', '', 'Функция закреплена: разрешаем только фото. Видео и файлы не включаем.', 'Тарифное ограничение будет использоваться как воронка продаж.'], [[btn('⚙️ Настройки','comments:settings'), btn('💬 В комментарии','comments:home')], ...nav('comments')]);
  if (o === 'comments_reactions') return screen(['❤️ Реакции и ответы', '', 'Функция активна в интерфейсе обсуждений: реакции и ответы внутри треда.', 'Отдельные toggles будут сохраняться в настройках поста/канала.'], [[btn('👀 Предпросмотр','comments:preview'), btn('💬 В комментарии','comments:home')], ...nav('comments')]);
  if (['editor','buttons','gifts','highlight','polls','billing','referrals'].includes(o)) return flowRoute(adminId, route, payload);
...
async function helpRoute(route) {
  ...
  return screen(['❓ Помощь', '', s[0], s[1], '', 'Каждая кнопка V3 теперь должна открывать действие: активную функцию, выбор поста, flow или честную заглушку «в разработке».'], [[btn('🏠 Главное меню','main:home')]]);
}
...
async function renderScreen(route = 'main:home', adminId = '', payload = {}) {
  ...
  if (r.endsWith(':home') && SECTIONS[owner(r)]) return home(r);
  if (r.endsWith(':main_menu') || r === 'help:main_menu') return main();
  if (r.endsWith(':section_home')) return home(`${owner(r)}:home`);
...
function install() {
  ...
  try {
    ...
    clean.renderScreen = renderScreen;
    clean.__adminkitV3MenuActionsAdapter = { runtimeVersion: RUNTIME, sourceMarker: SOURCE };
    patched = true;
...
function selfTest() {
  ...
  return { ok: installed && patched, runtimeVersion: RUNTIME, sourceMarker: SOURCE, installed, patched, routeCount, renderCount, lastRoute, lastRenderedAt, lastError, policy: { everyVisibleButtonHasHandler: true, oldPatchedPostsUntouchedByNavigation: true, openAppCommentsUntouched: true, devRoutesSayInDevelopment: true } };
}

module.exports = { RUNTIME, SOURCE, install, selfTest, renderScreen };
