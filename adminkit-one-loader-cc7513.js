...
const SOURCE = 'adminkit-cc7-5-13-button-menu-bottom-flow';
const MARKER = '__ADMINKIT_CC7_5_13_BUTTON_MENU_BOTTOM_FLOW_LOADER__';
...
function loadLayer(pathName) {
  ...
  catch (error) { item.error = error?.message || String(error); console.warn('[cc7.5.13] layer failed:', pathName, item.error); }
  loadedLayers.push(item);
  return item;
}
function installRoutes(app) {
  if (!app || app.__adminkitCc7513Routes) return app;
  app.__adminkitCc7513Routes = true;
  ...
  app.get('/debug/cc7', (req, res) => {
    ...
    res.json({ ok: true, runtimeVersion: RUNTIME, sourceMarker: SOURCE, marker: MARKER, installedAt,
      policy: 'button_menu_bottom_after_input_over_7512_keep_comments_and_gifts',
      ...
      adminFlow: (() => { try { return require('./adminkit-admin-flows-7513').selfTest(); } catch (e) { return { ok: false, error: e?.message || String(e) }; } })(),
      ...
      audit: { commentsCore: 'unchanged from accepted CC7.5.6', giftsCore: 'kept from CC7.5.12', buttonsCore: 'CC7.5.13 only changes post-user-input menu placement and wording in buttons flow', noExternalAppButton: true, noOverlay: true, optimizationNote: 'current code is stable but layered; next cleanup should merge admin-flow wrappers 7510-7513 into one core module after acceptance' },
      commentOpenStateRoute: require('./routes/commentOpenState').selfTest(), generatedAt: Date.now() });
  });
  app.get(['/debug/ping', '/debug/version', '/debug/build-info'], (req, res) => { noCache(res); res.json({ ok: true, runtimeVersion: RUNTIME, displayVersion: 'CC7.5.13', sourceMarker: SOURCE, buildInfo: readBuildInfo(), loadedLayers, generatedAt: Date.now() }); });
  return app;
}
function installExpressWrap() {
  if (Module.__adminkitCc7513ExpressWrap) return;
  Module.__adminkitCc7513ExpressWrap = true;
  const prev = Module._load;
  Module._load = function adminkitCc7513Load(request, parent, isMain) {
    const loaded = prev.apply(this, arguments);
    if (String(request) === 'express' && loaded && !loaded.__adminkitCc7513Wrapped) {
      function wrappedExpress(...args) { return installRoutes(loaded(...args)); }
      Object.setPrototypeOf(wrappedExpress, loaded);
      Object.assign(wrappedExpress, loaded);
      wrappedExpress.__adminkitCc7513Wrapped = true;
