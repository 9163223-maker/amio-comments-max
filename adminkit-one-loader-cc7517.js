...
const SOURCE = 'adminkit-cc7-5-17-gift-manager-like-buttons';
const MARKER = '__ADMINKIT_CC7_5_17_GIFT_MANAGER_LOADER__';
...
function loadLayer(pathName) {
  ...
  catch (error) { item.error = error?.message || String(error); console.warn('[cc7.5.17] layer failed:', pathName, item.error); }
  loadedLayers.push(item);
  return item;
}
function installRoutes(app) {
  if (!app || app.__adminkitCc7517Routes) return app;
  app.__adminkitCc7517Routes = true;
  ...
  app.get('/debug/cc7', (req, res) => {
    ...
    res.json({ ok: true, runtimeVersion: RUNTIME, sourceMarker: SOURCE, marker: MARKER, installedAt,
      policy: 'gift_manager_like_buttons_over_7516_keep_comments_and_buttons_core',
      ...
      adminFlow: (() => { try { return require('./adminkit-admin-flows-7517').selfTest(); } catch (e) { return { ok: false, error: e?.message || String(e) }; } })(),
      ...
      audit: { commentsCore: 'unchanged from accepted CC7.5.6', buttonsCore: 'unchanged from accepted CC7.5.16', giftsCore: 'new manager only: existing gifts detection, add/edit/delete, multi gift rows', patcher: 'DB-V3-POST-PATCHER-1.3-MULTI-GIFTS-PRESERVE', noExternalAppButton: true, noOverlay: true, optimizationNote: 'still layered; after acceptance merge admin flow wrappers 7510-7517 into one clean core module' },
      commentOpenStateRoute: require('./routes/commentOpenState').selfTest(), generatedAt: Date.now() });
  });
  app.get(['/debug/ping', '/debug/version', '/debug/build-info'], (req, res) => { noCache(res); res.json({ ok: true, runtimeVersion: RUNTIME, displayVersion: 'CC7.5.17', sourceMarker: SOURCE, buildInfo: readBuildInfo(), loadedLayers, generatedAt: Date.now() }); });
  return app;
}
function installExpressWrap() {
  if (Module.__adminkitCc7517ExpressWrap) return;
  Module.__adminkitCc7517ExpressWrap = true;
  const prev = Module._load;
  Module._load = function adminkitCc7517Load(request, parent, isMain) {
    const loaded = prev.apply(this, arguments);
    if (String(request) === 'express' && loaded && !loaded.__adminkitCc7517Wrapped) {
      function wrappedExpress(...args) { return installRoutes(loaded(...args)); }
      Object.setPrototypeOf(wrappedExpress, loaded);
      Object.assign(wrappedExpress, loaded);
      wrappedExpress.__adminkitCc7517Wrapped = true;
