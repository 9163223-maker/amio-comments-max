...
/**
 * CC5.4 comments clean shell + floating CTA + public post registration.
...
const RUNTIME = 'CC5.4';
const SOURCE = 'adminkit-CC5.4-comments-post-register-and-cta';
...
function install(app) {
  ...
  app.get('/public/app.js', (req, res, next) => {
    ...
    catch (error) { console.error('[CC5.4 comments shell]', error && error.message ? error.message : error); next(); }
  });
  app.get('/debug/comments-shell', (req, res) => {
    noCache(res);
    let size = 0;
    try { size = Buffer.byteLength(buildClientSource(), 'utf8'); } catch {}
    res.json({ ok: true, runtimeVersion: RUNTIME, sourceMarker: SOURCE, commentsShell: 'enabled', floatingCta: 'restored', publicPostRegister: 'client_enabled', legacyClientPatch: 'stripped_from_app_js_route', appJsBytes: size, generatedAt: Date.now() });
  });
  return app;
}
