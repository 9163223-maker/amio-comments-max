...
function safeContract() {
  ...
  catch (error) { return { runtime: runtimeContract.RUNTIME || 'RUNTIME-CONTRACT-PR196', sourceMarker: runtimeContract.SOURCE || 'adminkit-runtime-contract-pr196', safe: true, contractLiveOk: false, mismatches: ['runtime_contract_build_failed'], error: clean(error && error.message || error).slice(0, 160) }; }
}
function runtimeInfo() {
  ...
  return { startedAt, runtimeVersion: clean(buildInfo.runtimeVersion || process.env.RUNTIME_VERSION || process.env.BUILD_VERSION), buildVersion: clean(buildInfo.buildVersion || process.env.BUILD_VERSION || process.env.RUNTIME_VERSION), displayVersion: clean(buildInfo.displayVersion || process.env.BUILD_VERSION || process.env.RUNTIME_VERSION), sourceMarker: clean(buildInfo.sourceMarker || process.env.BUILD_SOURCE_MARKER), entrypoint: clean(buildInfo.activeEntrypoint || 'clean-entrypoint-1.53.10-pr89.js'), gitCommit: clean(buildInfo.gitCommit || process.env.GIT_COMMIT || process.env.COMMIT_SHA || process.env.RENDER_GIT_COMMIT || process.env.SOURCE_VERSION), pr188PushMultiChatHandoff: buildInfo.pr188PushMultiChatHandoff === true, pr191PushAdminInviteTitleCommands: buildInfo.pr191PushAdminInviteTitleCommands === true || process.env.ADMINKIT_PR191_PUSH_ADMIN_INVITE_TITLE_COMMANDS === '1', pr178PushPairingBinding: buildInfo.pr178PushPairingBinding === true || process.env.ADMINKIT_PR178_PUSH_PAIRING_BINDING === '1', pushPairingRuntimeVersion: clean(buildInfo.pushPairingRuntimeVersion || 'CC8.3.54-PR188-PUSH-MULTI-CHAT-HANDOFF'), pushRuntimeSourceMarker: clean(buildInfo.pushRuntimeSourceMarker || buildInfo.sourceMarker || process.env.BUILD_SOURCE_MARKER), pushPairingBaseSourceMarker: clean(buildInfo.pushPairingBaseSourceMarker || 'adminkit-pr188-push-multi-chat-handoff'), pushPairingSourceMarker: clean(buildInfo.pushPairingSourceMarker || buildInfo.sourceMarker || process.env.BUILD_SOURCE_MARKER), pr165RuntimeWired: buildInfo.pr165RuntimeWired === true || process.env.PR165_RUNTIME_WIRED === '1', postgresConfigured: Boolean(process.env.DATABASE_URL || process.env.POSTGRES_URL || process.env.POSTGRES_CONNECTION_STRING || process.env.PGHOST), canonicalPublicBaseUrl: clean(process.env.ADMINKIT_PUBLIC_BASE_URL || 'https://p01--amio-commnets-max--qkpwxnxqqrnw.code.run'), runtimeContract: safeContract(), liveVersionSnapshot, finalRuntimeReadinessGate: finalRuntimeReadinessGate(liveVersionSnapshot) };
}
function recordStartupNow(extra = {}) { const payload = { ...runtimeInfo(), ...extra }; if (!payload.finalRuntimeReadinessGate) payload.finalRuntimeReadinessGate = finalRuntimeReadinessGate(payload.liveVersionSnapshot); return startupLog.recordStartup(payload); }
function finalRuntimeReadinessGate(snapshot = {}) { try { const gate = require('./pr205-final-runtime-readiness-gate'); const current = gate && typeof gate.info === 'function' ? gate.info() : {}; if (current && current.finalRuntimeReadinessGate) return current.finalRuntimeReadinessGate; if (gate && typeof gate.buildGate === 'function') return gate.buildGate(snapshot); } catch {} if (startupLog && typeof startupLog.buildFinalRuntimeReadinessGateFromSnapshot === 'function') return startupLog.buildFinalRuntimeReadinessGateFromSnapshot(snapshot); return null; }
...
function isFinalDisabledProductionProbeStartup(info = {}) {
  ...
  return snapshot.ok === true && summary.pr199Ready === true && summary.pr202Ready === true && summary.plusSignWizardSupported === true && probe.pending === true && probe.ok !== true && diagnostics.includes('startup_production_probe_disabled') && gate && gate.ok !== true && gate.readyForManualMaxTest !== true && onlyExpectedMissing;
}
