...
const SOURCE = 'adminkit-cc7-5-12-gift-menu-addon-preserve';
const MARKER = '__ADMINKIT_CC7_5_12_GIFT_MENU_ADDON_PRESERVE_LOADER__';
...
function loadLayer(pathName) {
  ...
  catch (error) { item.error = error?.message || String(error); console.warn('[cc7.5.12] layer failed:', pathName, item.error); }
  loadedLayers.push(item);
  return item;
}
function installRoutes(app) {
  if (!app || app.__adminkitCc7512Routes) return app;
  app.__adminkitCc7512Routes = true;
  ...
  app.get('/debug/cc7', (req, res) => {
    ...
    res.json({
      ...
      marker: MARKER,
      installedAt,
      policy: 'gift_menu_bottom_after_input_plus_comment_refresh_preserves_buttons_and_gifts',
      ...
      adminFlow: (() => { try { return require('./adminkit-admin-flows-7512').selfTest(); } catch (e) { return { ok: false, error: e?.message || String(e) }; } })(),
      ...
      audit: {
        ...
        giftsCore: 'CC7.5.12 only fixes gift next-step menu placement after user input',
        addonRefresh: 'db-v3-post-patcher rebuilds comments keyboard with saved CTA/gift rows so commenting does not remove gift/button',
  ...
  app.get(['/debug/ping', '/debug/version', '/debug/build-info'], (req, res) => { noCache(res); res.json({ ok: true, runtimeVersion: RUNTIME, displayVersion: 'CC7.5.12', sourceMarker: SOURCE, buildInfo: readBuildInfo(), loadedLayers, generatedAt: Date.now() }); });
  return app;
}
function installExpressWrap() {
  if (Module.__adminkitCc7512ExpressWrap) return;
  Module.__adminkitCc7512ExpressWrap = true;
  const prev = Module._load;
  Module._load = function adminkitCc7512Load(request, parent, isMain) {
    const loaded = prev.apply(this, arguments);
    if (String(request) === 'express' && loaded && !loaded.__adminkitCc7512Wrapped) {
      function wrappedExpress(...args) { return installRoutes(loaded(...args)); }
      Object.setPrototypeOf(wrappedExpress, loaded);
      Object.assign(wrappedExpress, loaded);
      wrappedExpress.__adminkitCc7512Wrapped = true;
