...
const RUNTIME = 'ADMINKIT-POSTGRES-POST-ARCHIVE-1.0';
...
function getPoolConfig() {
  ...
  const base = {
    max: Number(process.env.ADMINKIT_ARCHIVE_PG_POOL_MAX || 2),
...
async function ensure() {
  ...
  ensurePromise = (async () => {
    ...
    try {
      await query(`CREATE TABLE IF NOT EXISTS ak_channels (
      ...
      await query(`CREATE TABLE IF NOT EXISTS ak_posts (
      ...
      for (const sql of postMigration) await query(sql);

      await query(`CREATE TABLE IF NOT EXISTS ak_post_snapshots (
        id BIGSERIAL PRIMARY KEY,
        channel_id TEXT NOT NULL DEFAULT '',
        post_id TEXT NOT NULL DEFAULT '',
        comment_key TEXT NOT NULL DEFAULT '',
        ...
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE(channel_id, post_id, snapshot_hash)
      )`);
      await query(`CREATE INDEX IF NOT EXISTS ak_post_snapshots_post_idx ON ak_post_snapshots(channel_id, post_id, created_at DESC)`);
      await query(`CREATE INDEX IF NOT EXISTS ak_post_snapshots_comment_key_idx ON ak_post_snapshots(comment_key, created_at DESC)`);

      await query(`CREATE TABLE IF NOT EXISTS ak_post_archive (
        id BIGSERIAL PRIMARY KEY,
        channel_id TEXT NOT NULL DEFAULT '',
        post_id TEXT NOT NULL DEFAULT '',
        comment_key TEXT NOT NULL DEFAULT '',
        archive_reason TEXT NOT NULL DEFAULT 'snapshot',
        post_title TEXT NOT NULL DEFAULT '',
        post_preview TEXT NOT NULL DEFAULT '',
        payload JSONB NOT NULL DEFAULT '{}'::jsonb,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
      )`);
      await query(`CREATE INDEX IF NOT EXISTS ak_post_archive_post_idx ON ak_post_archive(channel_id, post_id, created_at DESC)`);
      await query(`CREATE INDEX IF NOT EXISTS ak_post_archive_comment_key_idx ON ak_post_archive(comment_key, created_at DESC)`);
...
function channelTitle(channel = {}) {
  return clean(channel.title || channel.channelTitle || channel.name || channel.chatTitle || channel.channelName || '');
}

function postTitle(post = {}) {
  return clean(post.postTitle || post.title || post.originalText || post.postText || post.text || post.messageText || 'Пост').slice(0, 180);
}

function postPreview(post = {}) {
  return clean(post.postPreview || post.preview || post.originalText || post.postText || post.text || post.messageText || postTitle(post)).slice(0, 500);
}
...
function collectChannels(snapshot = {}) {
  const source = snapshot && typeof snapshot.channels === 'object' ? snapshot.channels : {};
  return Object.values(source).filter((item) => item && typeof item === 'object');
}

function collectPosts(snapshot = {}) {
  const source = snapshot && typeof snapshot.posts === 'object' ? snapshot.posts : {};
  return Object.entries(source)
    .map(([key, value]) => ({ commentKey: clean(value && value.commentKey || key), ...(value || {}) }))
    .filter((item) => clean(item.channelId) && clean(item.postId));
}
...
async function syncChannels(channels = []) {
  for (const ch of channels) {
    ...
    await query(
      `INSERT INTO ak_channels (channel_id, channel_title, linked_by_user_id, active, meta, created_at, updated_at)
       VALUES ($1, $2, $3, TRUE, $4::jsonb, NOW(), NOW())
       ON CONFLICT (channel_id) DO UPDATE SET
         channel_title = COALESCE(NULLIF(EXCLUDED.channel_title, ''), ak_channels.channel_title),
         linked_by_user_id = COALESCE(NULLIF(EXCLUDED.linked_by_user_id, ''), ak_channels.linked_by_user_id),
         active = TRUE,
         meta = ak_channels.meta || EXCLUDED.meta,
         updated_at = NOW()`,
      [channelId, channelTitle(ch), clean(ch.linkedByUserId || ch.adminId || ch.userId), JSON.stringify(ch)]
    );
  }
}
...
async function syncPosts(posts = [], reason = 'auto') {
  ...
  for (const post of posts) {
    ...
    await query(
      `INSERT INTO ak_posts (admin_id, channel_id, channel_title, post_id, message_id, comment_key, post_title, post_preview, source, archived, meta, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, FALSE, $10::jsonb, NOW(), NOW())
       ON CONFLICT (channel_id, post_id) DO UPDATE SET
         admin_id = COALESCE(NULLIF(EXCLUDED.admin_id, ''), ak_posts.admin_id),
         channel_title = COALESCE(NULLIF(EXCLUDED.channel_title, ''), ak_posts.channel_title),
         message_id = COALESCE(NULLIF(EXCLUDED.message_id, ''), ak_posts.message_id),
         comment_key = COALESCE(NULLIF(EXCLUDED.comment_key, ''), ak_posts.comment_key),
         post_title = COALESCE(NULLIF(EXCLUDED.post_title, ''), ak_posts.post_title),
         post_preview = COALESCE(NULLIF(EXCLUDED.post_preview, ''), ak_posts.post_preview),
         source = EXCLUDED.source,
         archived = FALSE,
         meta = ak_posts.meta || EXCLUDED.meta,
         updated_at = NOW()`,
      [clean(post.adminId || post.admin_id || post.linkedByUserId), channelId, clean(post.channelTitle || post.chatTitle || post.channelName), postId, clean(post.messageId), commentKey, title, preview, reason, JSON.stringify(payload)]
    );
    upserted += 1;
    const inserted = await query(
      `INSERT INTO ak_post_snapshots (channel_id, post_id, comment_key, snapshot_hash, snapshot_kind, post_title, post_preview, payload, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8::jsonb, NOW())
       ON CONFLICT (channel_id, post_id, snapshot_hash) DO NOTHING
       RETURNING id`,
    ...
    if (inserted.rowCount > 0) {
      ...
      await query(
        ...
        [channelId, postId, commentKey, reason, title, preview, JSON.stringify(payload)]
      );
    }
  }
...
function scheduleSync(snapshot = {}, options = {}) {
  ...
  pendingTimer = setTimeout(() => {
    ...
    pendingPromise = syncStore(next || {}, { reason }).catch((error) => {
      lastInfo = { ...lastInfo, ok: false, configured: true, lastError: String(error && error.message || error) };
    });
  }, delayMs);
  return true;
