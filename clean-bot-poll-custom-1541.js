...
function mid(m){return String((m&&m.body&&(m.body.mid||m.body.message_id))||m&&m.message_id||m&&m.messageId||m&&m.id||'').trim()}
function chat(m){return String((m&&m.recipient&&(m.recipient.chat_id||m.recipient.id))||m&&m.chat_id||m&&m.chat&&m.chat.id||'').trim()}
...
function payloadValue(c){if(!c)return'';return c.payload!==undefined?c.payload:c.data!==undefined?c.data:c.value!==undefined?c.value:c.callback_data!==undefined?c.callback_data:c.callbackData!==undefined?c.callbackData:'';}
function raw(c){const v=payloadValue(c);if(!v)return'';return typeof v==='object'?JSON.stringify(v):String(v).trim();}
function parse(c){const v=payloadValue(c);if(v&&typeof v==='object')return v;const r=raw(c);if(!r)return{};try{return JSON.parse(r)}catch{return{action:r,raw:r}}}
...
function errorScreen(text){return{id:'poll_custom_error',text:['⚠️ Не удалось открыть настройку опроса','',String(text||'unknown')].join('\n'),attachments:menu.keyboard([[menu.button('🗳 В начало опросов','admin_section_polls')],[menu.button('🏠 Главное меню','admin_section_main')]])};}
