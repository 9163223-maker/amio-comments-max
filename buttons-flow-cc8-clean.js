...
async function picker(menu, payload = {}, ctx = {}) { const page = Math.max(0, Number(payload.page || 0)); let channelId = clean(payload.channelId || ''); const channels = await listChannelsFromPosts(ctx.userId, ctx.config || {}); if (!channelId && channels.length > 1 && clean(payload.skipChannels || '') !== '1') return channelPicker(menu, payload, ctx); if (!channelId && channels.length === 1) channelId = clean(channels[0].channelId); if (!channelId && !channels.length) return screen(menu, 'buttons_clean_no_channels', '📌 Выбор поста для кнопок', ['У вас пока нет подключённых каналов.'], [[button(menu, 'Подключить канал', 'admin_bind_channel')], [button(menu, 'Главное меню', 'admin_section_main')]]); const posts = await listPostsForChannel(channelId, ctx.userId); if (channelId && posts[0]) await hydrateChannelTitle(posts[0], ctx); const rows = posts.map((post, index) => [button(menu, `${index + 1 + page * MAX_POSTS}. ${pickerCore.safePostPreview(post)}`, 'button_admin_select_post', { commentKey: clean(post.commentKey), channelId, postId: clean(post.postId) })]); if (!rows.length) rows.push([button(menu, 'В этом канале пока нет сохранённых постов.', 'admin_section_buttons')]); rows.push(...footer(menu)); const resolvedTitle = channelId ? (channels.find((item) => clean(item.channelId) === channelId)?.title || channelTitle(posts[0] || { channelId }, ctx.userId)) : ''; const lines = [channelId ? `Канал: ${resolvedTitle}` : '', 'Выберите пост.'].filter(Boolean); if (posts.length) { posts.forEach((post, index) => { const meta = [postTime(post)].filter(Boolean).join(' · '); lines.push(`${index + 1}. ${pickerCore.safePostPreview(post)}${meta ? '\n   ' + meta : ''}`); }); } else if (channelId) lines.push('В этом канале пока нет сохранённых постов.'); return screen(menu, 'buttons_clean_picker', '📌 Выбор поста для кнопок', lines, rows); }
async function selectPost(menu, payload = {}, ctx = {}) { let post = await findPostForSelection(payload.commentKey || '', ctx.userId, payload.channelId || ''); if (!post || !post.commentKey) return screen(menu, 'buttons_clean_not_found', '🔘 Кнопки под постами', ['Пост не найден для выбранного канала.'], [[button(menu, '📌 Выбрать пост', 'button_admin_recent_posts', { page: 0 })], ...footer(menu)]); post = await hydrateChannelTitle(post, ctx); bindTarget(ctx.userId, post); const card = bindCurrentCard(ctx.userId, post); const reconciled = reconcileButtonsFeatureState({ userId: ctx.userId, target: post }); const imported = reconciled.buttons; const out = selectedCardScreen(menu, post, card, imported, imported.length ? `Пост для кнопок выбран. Найдены текущие кнопки: ${imported.length}.` : 'Пост для кнопок выбран. Кнопок пока нет.', ctx.userId); out.__trace = { context: { userId: ctx.userId, target: post, ok: true, buttons: imported, imported: reconciled.imported, buttonSource: reconciled.source, diagnostics: reconciled.diagnostics } }; return out; }
...
async function resolveSelectedButtonsContextReadOnly(ctx = {}, payload = {}) {
  ...
  const canonical = getButtonSet(target, userId);
  const diagnostics = [{ source: 'canonical_button_set', count: canonical.length, readOnly: true }];
  return { ok: true, readOnly: true, userId, tenantKey: tenantCtx.tenantKey, ownerUserId: tenantCtx.ownerUserId, channelId: clean(target.channelId || target.requiredChatId), channelTitle: channelTitle(target, userId), postId: clean(target.postId), commentKey: clean(target.commentKey), postTitle: postTitle(target), target, buttons: canonical, buttonSource: canonical.length ? 'canonical_button_set' : 'empty', imported: false, diagnostics, sourceDiagnostics };
}
async function resolveSelectedButtonsContext(ctx = {}, payload = {}) {
  const context = await resolveSelectedButtonsContextReadOnly(ctx, payload);
  if (!context.ok) return context;
  const target = bindTarget(context.userId, context.target, { keepFlow: Boolean(getSetup(context.userId).buttonFlow) });
  const reconciled = reconcileButtonsFeatureState({ userId: context.userId, target });
  return { ...context, readOnly: false, target, buttons: reconciled.buttons, buttonSource: reconciled.source, imported: reconciled.imported, diagnostics: reconciled.diagnostics };
}
