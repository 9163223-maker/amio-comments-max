...
function selectedCardRows(menu, card = null, buttons = []) { const rows = [[button(menu, '➕ Добавить кнопку к этому посту', 'button_admin_start_add', { cardId: clean(card?.cardId) })], [button(menu, '📋 Текущие кнопки', 'button_admin_show_current', { cardId: clean(card?.cardId) })]]; if (array(buttons).length) rows.push([button(menu, '🗑 Удалить последнюю кнопку', 'button_admin_delete', { source: 'current_buttons_card' })]); rows.push([button(menu, '📌 Выбрать другой пост', 'button_admin_recent_posts', { page: 0 })], ...footer(menu)); return rows; }
...
function selectedPostLines(target = null, buttons = [], userId = '') { const title = postTitle(target); const count = array(buttons).length; return [`Пост выбран: «${title}»`, ...targetLines(target, userId), '', `Текущие кнопки (${count}):`, ...buttonsLines(buttons), '', count ? 'Что сделать?' : 'Кнопок пока нет. Добавьте первую кнопку для этого поста.']; }
