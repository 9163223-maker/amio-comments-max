...
function selectedCardRows(menu, card = null, buttons = []) { const rows = [[button(menu, '➕ Добавить кнопку', 'button_admin_start_add', { cardId: clean(card?.cardId) })], [button(menu, '📋 Текущие кнопки', 'button_admin_show_current', { cardId: clean(card?.cardId) })]]; if (array(buttons).length) rows.push([button(menu, '🗑 Удалить последнюю кнопку', 'button_admin_delete', { source: 'current_buttons_card' })]); rows.push([button(menu, '⬅️ Назад к постам', 'button_admin_recent_posts', { page: 0 })], ...footer(menu)); return rows; }
function selectedPostLines(target = null, buttons = []) { const title = postTitle(target); const count = array(buttons).length; return [`Post selected: “${title}”`, '', `Current buttons (${count}):`, ...buttonsLines(buttons), '', count ? 'What do you want to do?' : 'Кнопок пока нет. Добавьте первую кнопку для этого поста.']; }
function selectedCardScreen(menu, target = null, card = null, buttons = [], note = '') { return screen(menu, 'buttons_clean_selected_post', '🔘 Пост для кнопок выбран', [clean(note), ...selectedPostLines(target, buttons)].filter(Boolean), selectedCardRows(menu, card, buttons)); }
