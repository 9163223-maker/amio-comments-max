...
function selectedPostLines(target = null, buttons = []) { const title = postTitle(target); const count = array(buttons).length; return [`Пост выбран: «${title}»`, '', `Текущие кнопки (${count}):`, ...buttonsLines(buttons), '', count ? 'Что сделать?' : 'Кнопок пока нет. Добавьте первую кнопку для этого поста.']; }
