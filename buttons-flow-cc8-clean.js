...
function selectedCardRows(menu, card = null, buttons = []) { const count = array(buttons).length; const cardId = clean(card?.cardId); const rows = []; rows.push([button(menu, count ? '➕ Добавить ещё кнопку' : '➕ Добавить кнопку', 'button_admin_start_add', { cardId })]); if (count === 1) { const data = { cardId, buttonId: buttonIdAt(buttons, 0) }; rows.push([button(menu, '✏️ Изменить кнопку', 'button_admin_edit', data)]); rows.push([button(menu, '🗑 Удалить кнопку', 'button_admin_delete_confirm', data)]); } else if (count > 1) { rows.push([button(menu, '✏️ Изменить кнопку', 'button_admin_edit', { cardId })]); rows.push([button(menu, '🗑 Удалить кнопку', 'button_admin_delete_confirm', { cardId })]); } rows.push([button(menu, '📌 Выбрать другой пост', 'button_admin_recent_posts', { page: 0 })], [button(menu, '🏠 Главное меню', 'admin_section_main')]); return rows; }
...
async function showCurrent(menu, payload = {}, ctx = {}) { let target = await currentTarget(payload, ctx); if (target) target = await hydrateChannelTitle(target, ctx); if (!target || !target.commentKey) return picker(menu, { page: 0 }, ctx); target = bindTarget(ctx.userId, target); const card = bindCurrentCard(ctx.userId, target); const buttons = getButtonSet(target, ctx.userId); return selectedCardScreen(menu, target, card, buttons, '', ctx.userId); }
...
async function selectedTargetFromCardOrStore(payload = {}, ctx = {}) {
  const card = getCurrentCard(ctx.userId);
  const payloadCardId = clean(payload.cardId || payload.currentCardId || '');
  if (payloadCardId && (!card || payloadCardId !== clean(card.cardId))) return null;
  const source = card || await currentTarget(payload, ctx);
  return source ? hydrateChannelTitle(source, ctx) : null;
}
async function renderEdit(menu, payload = {}, ctx = {}) {
  const target = await selectedTargetFromCardOrStore(payload, ctx);
  if (!target || !target.commentKey) return picker(menu, { page: 0 }, ctx);
  bindTarget(ctx.userId, target);
  const buttons = getButtonSet(target, ctx.userId);
  ...
  if (buttons.length > 1 && index < 0) {
    ...
    rows.push([button(menu, '↩️ Назад', 'button_admin_show_current')], [button(menu, '🏠 Главное меню', 'admin_section_main')]);
...
async function removeLast(menu, payload = {}, ctx = {}) { let target = await selectedTargetFromCardOrStore(payload, ctx) || getStoredTarget(ctx.userId); if (target) target = await hydrateChannelTitle(target, ctx); const existing = getButtonSet(target, ctx.userId); if (!existing.length) return home(menu, { note: 'Кнопок для удаления нет.' }, ctx); const index = findButtonIndex(existing, payload.buttonId); if (index < 0) return renderDeleteConfirm(menu, payload, ctx); const next = existing.filter((_, i) => i !== index); saveButtonSet(target, next, ctx.userId); const patch = await repatchTarget(target, ctx); bindTarget(ctx.userId, target); return selectedCardScreen(menu, target, bindCurrentCard(ctx.userId, target), getButtonSet(target, ctx.userId), patchNote(patch, 'Кнопка удалена'), ctx.userId); }
