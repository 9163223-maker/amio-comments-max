...
async function showCurrent(menu, payload = {}, ctx = {}) { let target = await currentTarget(payload, ctx); if (target) target = await hydrateChannelTitle(target, ctx); if (!target || !target.commentKey) return picker(menu, { page: 0 }, ctx); target = bindTarget(ctx.userId, target); const buttons = getButtonSet(target, ctx.userId); return screen(menu, 'buttons_clean_current', '📋 Текущие кнопки', [...targetLines(target, ctx.userId), '', ...buttonsLines(buttons)], currentRows(menu, target, buttons)); }
