...
const RUNTIME = 'CC8.3.7-PR217-DB-BOUND-BUTTONS-SAVE-CURRENT';
const MAX_POSTS = 8;
...
function buttonsFromStoredPost(target = null, userId = '') { const post = findPost(clean(target && target.commentKey), userId) || safeCall(() => store.getPost(clean(target && target.commentKey)), null) || null; const rows = array(post && post.customKeyboard && post.customKeyboard.rows); const buttons = []; rows.forEach((row) => array(row.buttons).forEach((btn) => { if (clean(btn.text) && clean(btn.url || btn.href || btn.targetUrl)) buttons.push(tenant.stampRecord({ id: clean(btn.id), text: clean(btn.text), url: normalizeButtonUrl(btn.url || btn.href || btn.targetUrl), commentKey: clean(target.commentKey), channelId: clean(target.channelId || target.requiredChatId), source: 'canonical_stored_post_customKeyboard' }, getTenant(userId), btn)); })); return buttons; }
function getButtonSet(target = null, userId = '') { if (!target || !target.channelId || !target.commentKey) return []; const fromPost = buttonsFromStoredPost(target, userId); if (fromPost.length) return fromPost; const ctx = getTenant(userId); const set = channelStore(target.channelId).buttonSets[clean(target.commentKey)] || []; return array(set).filter((item) => tenant.belongsToTenant(item, ctx)); }
