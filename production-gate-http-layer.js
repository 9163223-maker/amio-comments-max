...
const RUNTIME = 'ADMINKIT-PRODUCTION-GATE-HTTP-LAYER-1.51.4';
const MARKER = '__ADMINKIT_PRODUCTION_GATE_HTTP_LAYER_1_51_4__';

let checklistCache = null;
let debugExportCache = null;

function getChecklist() {
  if (!checklistCache) checklistCache = require('./src/core/productionChecklistAdapter');
  return checklistCache;
}

function getDebugExport() {
  if (!debugExportCache) debugExportCache = require('./src/core/debugExportAdapter');
  return debugExportCache;
}
...
function payloadFor(pathname = '') {
  ...
  if (pathname === '/debug/production-features') {
    ...
    return {
      ok: rows.every((x) => x.ok === true),
      runtimeVersion: RUNTIME,
      generatedAt: new Date().toISOString(),
      mode: 'production-features-http-compact',
      total: rows.length,
      failed: rows.filter((x) => x.ok !== true).map(compactItem),
      features: rows.map((x) => ({
        id: x.id,
        title: x.title,
        ok: x.ok,
  ...
  if (pathname === '/debug/production-security') {
    ...
    return {
      ok: rows.every((x) => x.status !== 'blocker'),
      runtimeVersion: RUNTIME,
      generatedAt: new Date().toISOString(),
      mode: 'production-security-http-compact',
      total: rows.length,
      blockers: rows.filter((x) => x.status === 'blocker').map(compactItem),
  ...
  if (pathname === '/debug/production-env') {
    ...
    return {
      ...
      runtimeVersion: RUNTIME,
      generatedAt: new Date().toISOString(),
      mode: 'production-env-http-compact',
      total: rows.length,
      blockers: rows.filter((x) => x.status === 'blocker').map(compactItem),
      warnings: rows.filter((x) => x.status === 'warning').map(compactItem),
      checks: rows.map(compactItem),
      secretsHidden: true
    };
  }
  ...
  if (pathname === '/debug/production-gate-selftest') {
    const debugExport = getDebugExport();
    return {
      ok: true,
      runtimeVersion: RUNTIME,
      generatedAt: new Date().toISOString(),
  ...
  return {
    ...
    generatedAt: new Date().toISOString(),
    mode: 'production-gate-http-compact',
    ...
    httpLayer: true,
    note: 'Короткий HTTP-layer endpoint. Не использует общий /debug/core-stress и не зависит от Express route registration.'
  };
}
...
function isProductionGateUrl(url = '') {
  ...
  return [
    '/debug/production-gate',
    '/debug/production-features',
    '/debug/production-security',
    '/debug/production-env',
    '/debug/production-gate-selftest'
  ].includes(pathname);
}
    ...
    if (originalListener) {
      args[0] = function adminkitProductionGateRequestListener(req, res) {
        if (isProductionGateUrl(req.url)) {
          try {
            return sendJson(res, payloadFor(String(req.url || '').split('?')[0]));
          } catch (error) {
            return sendJson(res, {
              ok: false,
              runtimeVersion: RUNTIME,
...
function selfTest(already = false) {
  return {
    ...
    marker: MARKER,
    already,
    httpCreateServerWrapped: true,
    compactOnly: true,
    lazyRequire: true,
    autoInstallOnPreload: true,
...
try {
  if (process.env.ADMINKIT_DISABLE_PRODUCTION_GATE_HTTP_LAYER !== '1') install();
} catch (error) {
  console.error('[adminkit] production gate http layer install failed:', error?.message || error);
}
