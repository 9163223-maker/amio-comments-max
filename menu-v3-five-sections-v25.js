...
const RUNTIME = 'HARD-V3-ADMIN-MENU-2.5-SIMPLE-MODERATION-LIVE';
const mem = new Map();
...
async function listPosts(id, channelId = '') { const out = []; try { out.push(...(await db.getPosts(id, channelId, 80))); } catch {} try { (store.getPostsList ? store.getPostsList() : []).forEach(p => { if (!channelId || String(p.channelId || '') === String(channelId)) out.push({ postId: p.postId, commentKey: p.commentKey, title: p.title || p.originalText || p.text || p.caption || '' }); }); } catch {} const seen = new Set(); return out.filter(p => { const k = clean(p.commentKey || p.postId || p.messageId); if (!k || seen.has(k)) return false; seen.add(k); return !isMenuText(p.title || p.text || ''); }).slice(0, 50); }
...
async function syncModeration(id, s = null) {
  ...
  const next = {
    ...(store.store?.moderation?.byChannel?.[channelId] || {}),
    enabled: state.moderationEnabled !== false,
    basicEnabled: true,
    stopwordsEnabled: state.moderationStopwordsEnabled !== false,
    applyPresetCommon: false,
    customBlocklist: words,
    blockLinks: state.moderationLinksAllowed === false,
    blockInvites: state.moderationInvitesAllowed !== false,
    aiEnabled: !!state.moderationAi,
    action: 'reject',
    updatedAt: Date.now()
  };
...
async function selectPost(id, sec, p = {}) { const t = isHuman(p.title) ? cut(p.title) : 'Пост выбран'; const patch = { selectedPostId: clean(p.postId), selectedCommentKey: clean(p.commentKey), selectedPostTitle: t }; if (sec === 'gifts') Object.assign(patch, { giftPostId: patch.selectedPostId, giftPostTitle: t }); if (sec === 'buttons') Object.assign(patch, { buttonsPostId: patch.selectedPostId, buttonsPostTitle: t }); await saveState(id, patch); }
...
async function renderAsync(rawRoute = 'main:home', id = '', p = {}) {
  ...
  if (route.includes(':toggle:')) { const key = route.split(':toggle:')[1]; s = await saveState(id, { [key]: toggleValue(s, key) }); if (sec === 'moderation') await syncModeration(id, s); return sectionHome(sec, s, ch); }
...
function selfTest() { return { ok: true, runtimeVersion: RUNTIME, readySections: ['channels', 'comments', 'gifts', 'buttons', 'moderation'], mainButtons: MAIN.length, simplifiedMenu: true, liveModerationSync: true, customStopwordsBlockComments: true, menuDelivery: 'send_new_then_delete_previous_and_current_callback_message', patcherTouched: false, commentsUiTouched: false, postgresUsed: true }; }
