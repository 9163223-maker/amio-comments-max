...
function directUrl(ads, campaign) { return ads.campaignUrl ? ads.campaignUrl(campaign) : clean(campaign && (campaign.targetUrl || campaign.url)); }
...
function copyScreen(menu, ads, campaign, ctx = {}) {
  ...
  return {
    ...
    text: [
      ...
      '',
      statUrl ? '📊 Ссылка для статистики / Яндекс.Кликера:' : '',
      statUrl || '',
      statUrl ? 'Сначала АдминКИТ фиксирует клик, потом переводит человека в MAX. Эту ссылку можно вставить в clck.ru для теста.' : '📊 Ссылка статистики пока недоступна: не задан публичный домен АдминКИТ.',
      '',
      '🔒 Прямая MAX-ссылка:',
      maxUrl,
      'Используйте как надёжный fallback. Точные клики по источнику по прямой ссылке не считаются.',
      '',
...
function install() {
  ...
  ads.selftest = function selftestWithTrackingLinks(config = {}) {
    ...
    return { ...base, runtimeVersion: RUNTIME, campaigns, trackingLinksSupported: true, trackingRedirectRoute: '/r/:slug', copyScreenShowsTrackingAndDirect: true };
  };
  ...
  return { ok: true, runtimeVersion: RUNTIME, trackingLinksSupported: true };
}
