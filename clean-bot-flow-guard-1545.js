...
function sender(m){ return String(m?.sender?.user_id || m?.sender?.id || m?.user?.user_id || m?.user?.id || m?.user_id || m?.userId || '').trim(); }
...
function actorLike(o){ return o&&typeof o==='object'&&!o.chat_id&&!o.chatId&&!o.chat_type&&!o.chatType&&!o.recipient&&!o.chat&&(o.user_id||o.userId||o.sender_id||o.senderId||o.from_id||o.fromId); }
function uid(u,c,m){ return userFrom(c)||userFrom(u)||sender(m)||userFrom(find(u,x=>actorLike(x),7)); }
...
function isPoll(p){ const a=String(p?.action||''), s=String(p?.source||''); return a==='poll_vote'||a==='poll_info'||a==='poll_status'||a==='poll_create'||a==='poll_custom_start'||a==='poll_custom_cancel'||a==='poll_custom_edit_question'||a==='poll_custom_edit_options'||a==='poll_custom_run'||a==='poll_results'||a==='poll_stop'||a==='admin_section_polls'||(a==='comments_select_post'&&s==='polls')||(a==='comments_pick_post'&&s==='polls'); }
...
async function pollScreen(p,userId,config){ const a=String(p.action||''); return timing.measure('poll_screen_build',{action:a,userId:timing.mask(userId)},async()=>{ if(a==='poll_custom_start')return polls.customStart(menu,{userId,commentKey:p.commentKey||'',source:p.source||''}); if(a==='poll_custom_cancel')return polls.customCancel(menu,{userId,commentKey:p.commentKey||''}); if(a==='poll_custom_edit_question')return polls.customEditQuestion(menu,{userId,commentKey:p.commentKey||''}); if(a==='poll_custom_edit_options')return polls.customEditOptions(menu,{userId,commentKey:p.commentKey||''}); if(a==='poll_custom_run')return polls.customRun(menu,{config,userId,commentKey:p.commentKey||''}); if(a==='poll_status')return polls.statusScreen(menu,{userId}); if(a==='poll_results')return polls.resultsScreen(menu,{userId,pollId:p.pollId||''}); if(a==='poll_stop')return polls.stopPoll(menu,{config,userId,pollId:p.pollId||'',source:p.source||''}); if(a==='admin_section_polls')return polls.home(menu,userId); if(a==='comments_select_post'&&String(p.source||'')==='polls')return polls.picker(menu,userId,p.channelId||''); if(a==='comments_pick_post'&&String(p.source||'')==='polls')return polls.picked(menu,p.commentKey||'',userId); if(a==='poll_create')return polls.createPoll(menu,{config,userId,commentKey:p.commentKey||'',template:p.template||'yes_no',source:p.source||''}); return null; }); }
