...
function sectionHome(owner) {
  ...
  if (section.id === 'comments') {
    ...
    return {
      ...
      route: section.route,
      owner: section.id,
      text: ['Комментарии', '', 'Настройте комментарии в каналах:', 'автоматически для новых постов или вручную для нужной публикации.', '', 'Пост не выбран. Сначала выберите канал, затем пост.'].join('\n'),
...
function pushHome() {
  return {
    ...
    route: 'push:home',
    owner: 'push',
    text: ['🔔 Уведомления MAX', '', 'Опубликуйте кнопку подключения в нужном MAX-чате или канале.', 'Участник нажмёт кнопку, а бот отправит ему персональную ссылку в личные сообщения.'].join('\n'),
    attachments: keyboard([
