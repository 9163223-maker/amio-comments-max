...
const RUNTIME = 'HARD-V3-ADMIN-MENU-2.3-CLIENT-FRIENDLY';
const mem = new Map();
...
const isHuman = (v) => !!clean(v) && !isTechnical(v) && clean(v) !== 'Канал не выбран' && clean(v) !== 'Пост не выбран';
const showChannel = (ch = {}, s = {}) => isHuman(ch.title) ? ch.title : isHuman(s.activeChannelTitle) ? s.activeChannelTitle : (ch.id ? 'Подключённый канал' : 'Канал не выбран');
const showPost = (s = {}) => {
  const title = clean(s.selectedPostTitle);
  if (isHuman(title)) return cut(title, 80);
  if (s.selectedPostId || s.selectedCommentKey) return 'Пост выбран';
  return 'не выбран';
};
const postTitle = (p = {}, fallback = 'Пост') => {
  const t = clean(p.title || p.originalText || p.text || p.caption || '');
  if (isHuman(t)) return cut(t, 42);
...
const LABELS = {
  commentsBannerText: 'текст баннера', commentsBannerButton: 'текст кнопки баннера', commentsBannerLink: 'ссылка или действие баннера',
  giftTitle: 'название подарка', giftLink: 'ссылка или файл подарка', giftMessage: 'сообщение получателю подарка',
  ctaButtonText: 'текст кнопки под постом', ctaButtonLink: 'ссылка или действие кнопки',
  moderationRules: 'правила модерации', moderationStopwords: 'стоп-слово'
};
const ALIAS = {
...
function normalizeRoute(route = '') { const r = clean(route); if (START_WORDS.has(r.toLowerCase())) return 'main:home'; return ALIAS[r] || r || 'main:home'; }
...
function addChannel(map, raw = {}) {
  ...
  if (!id) return;
  const title = clean(raw.title || raw.channelTitle || raw.name || raw.caption || '');
  const current = map.get(id);
  if (!current || (isHuman(title) && !isHuman(current.title))) map.set(id, { id, title: isHuman(title) ? title : current?.title || '' });
}
async function listChannels(id) {
  ...
  return [...map.values()].map(c => ({ ...c, title: isHuman(c.title) ? c.title : 'Подключённый канал' }));
}
async function activeChannel(id) {
  const s = await getState(id); const list = await listChannels(id);
  const found = list.find(x => x.id === s.activeChannelId) || list[0] || { id: '', title: '' };
...
async function listPosts(id, channelId = '') {
  ...
  try { (store.getPostsList ? store.getPostsList() : []).forEach(p => { if (!channelId || String(p.channelId || '') === String(channelId)) out.push({ postId: p.postId, commentKey: p.commentKey, title: p.title || p.originalText || p.text || p.postId }); }); } catch {}
  const seen = new Set();
...
function keyboard(items, route = 'main:home') { const rows = []; for (let i = 0; i < items.length; i += 2) rows.push(items.slice(i, i + 2).map(x => btn(x[0], x[1], x[2] || {}))); const o = owner(route); if (o !== 'main') rows.push([btn('❓ Помощь', 'help:home'), btn('↩️ Раздел', o + ':home')], [btn('🏠 Главное меню', 'main:home')]); return [{ type: 'inline_keyboard', payload: { buttons: rows } }]; }
function screen(title, lines, items = [], route = 'main:home') { const text = [title, '', Array.isArray(lines) ? lines.filter(Boolean).join('\n') : String(lines || '')].join('\n'); return { text, attachments: keyboard(items, route) }; }
function mainScreen() { return screen('🐋 АдминКИТ', ['Главное меню.', 'Выберите, что настроить: канал, комментарии, подарки, кнопки или модерацию.'], MAIN_BUTTONS, 'main:home'); }
...
function commentsStatus(s, ch) { const b = bannerState(s); return commonStatus(s, ch).concat([`Комментарии: ${status(s.commentsEnabled)}`, `Авто для новых постов: ${status(s.commentsAutoNew)}`, `Фото в комментариях: ${status(s.commentsPhoto)}`, `Реакции и ответы: ${status(s.commentsReactions)}`, `Баннер: ${b.enabled ? 'включен' : 'выключен'}`]); }
function giftsStatus(s, ch) { return commonStatus(s, ch).concat([`Подарки: ${status(s.giftsEnabled, false)}`, `Выдавать только подписчикам: ${status(s.giftsRequireSubscription)}`, `Запрашивать контакт: ${status(s.giftsContactRequired, false)}`, `Название подарка: ${clean(s.giftTitle) || 'не задано'}`, `Ссылка/файл подарка: ${clean(s.giftLink) ? 'задано' : 'не задано'}`]); }
function buttonsStatus(s, ch) { return commonStatus(s, ch).concat([`Кнопка под постом: ${status(s.buttonsEnabled, false)}`, `Текст кнопки: ${clean(s.ctaButtonText) || 'не задан'}`, `Ссылка/действие: ${clean(s.ctaButtonLink) ? 'задано' : 'не задано'}`, `Запрос контакта: ${status(s.buttonsRequestContact, false)}`, `Запрос гео: ${status(s.buttonsRequestGeo, false)}`]); }
function moderationStatus(s, ch) { return ['Статус:', `Канал: ${showChannel(ch, s)}`, `Модерация: ${status(s.moderationEnabled)}`, `Стоп-слова: ${status(s.moderationStopwordsEnabled)}`, `Ссылки: ${s.moderationLinksAllowed === false ? 'запрещены' : 'разрешены'}`, `Инвайты: ${s.moderationInvitesAllowed === false ? 'запрещены' : 'разрешены'}`, `AI-фильтр: ${status(s.moderationAi, false)}`, `Правила: ${clean(s.moderationRules) || 'не заданы'}`, `Стоп-слова: ${clean(s.moderationStopwords) || 'список пуст'}`]; }
...
async function postPicker(id, sec) { const ch = await activeChannel(id); const list = await listPosts(id, ch.id); return screen('📌 Выбрать пост', list.length ? [`Найдено постов: ${list.length}`, 'Нажмите нужный пост.'] : ['Постов пока нет.', 'Для старого поста нажмите «Старый пост» и перешлите публикацию.'], list.slice(0, 10).map((p, i) => [`${i + 1}. ${postTitle(p, 'Пост')}`, `${sec}:post`, { postId: p.postId, commentKey: p.commentKey, title: postTitle(p, 'Пост выбран') }]), `${sec}:posts`); }
async function setSelectedPost(id, sec, p = {}) { const title = isHuman(p.title) ? cut(p.title, 90) : 'Пост выбран'; const patch = { selectedPostId: clean(p.postId), selectedCommentKey: clean(p.commentKey), selectedPostTitle: title }; if (sec === 'gifts') Object.assign(patch, { giftPostId: clean(p.postId), giftPostTitle: title }); if (sec === 'buttons') Object.assign(patch, { buttonsPostId: clean(p.postId), buttonsPostTitle: title }); await saveState(id, patch); }
...
function missingButtonFields(s) { const m = []; if (!s.selectedPostId && !s.selectedCommentKey) m.push('выберите пост'); if (!clean(s.ctaButtonText)) m.push('введите текст кнопки'); if (!clean(s.ctaButtonLink) && !s.buttonsRequestContact && !s.buttonsRequestGeo) m.push('добавьте ссылку/действие или включите запрос контакта/гео'); return m; }
function missingGiftFields(s) { const m = []; if (!s.selectedPostId && !s.selectedCommentKey) m.push('выберите пост'); if (!clean(s.giftTitle)) m.push('введите название подарка'); if (!clean(s.giftLink)) m.push('добавьте ссылку или файл подарка'); return m; }
...
async function renderAsync(rawRoute = 'main:home', id = '', p = {}) {
  ...
  if (route.includes(':input:')) { const key = route.split(':input:')[1]; await saveState(id, { mode: `await:${sec}:${key}` }); return screen('✏️ Нужно ввести данные', [`Пришлите следующим сообщением: ${LABELS[key] || 'значение'}.`, 'После этого я сохраню настройку и верну вас в раздел.'], [], `${sec}:home`); }
  ...
  if (route === 'channels:list') { const list = await listChannels(id); return screen('📋 Мои каналы', list.length ? [`Найдено каналов: ${list.length}`, 'Нажмите канал, чтобы сделать его активным.'] : ['Каналы пока не найдены.', 'Перешлите боту пост из нужного канала.'], list.slice(0, 10).map((c, i) => [`${i + 1}. ${showChannel(c, s)}`, 'channels:select', { channelId: c.id, channelTitle: showChannel(c, s) }]), 'channels:list'); }
  if (route === 'channels:select') { const list = await listChannels(id); const c = list.find(x => x.id === clean(p.channelId)); const title = showChannel(c || {}, { activeChannelTitle: p.channelTitle }); if (c) await saveState(id, { activeChannelId: c.id, activeChannelTitle: title }); return screen('🔁 Активный канал', c ? [`Готово. Теперь работаем с каналом: ${title}.`] : 'Канал не найден.', [['📺 Каналы', 'channels:home']], 'channels:active'); }
  if (route === 'channels:connect') { await saveState(id, { mode: 'await_channel_forward', section: 'channels' }); return screen('➕ Подключить канал', ['Перешлите сюда любой пост из канала.', 'После этого канал появится в списке, а бот сможет настраивать комментарии, кнопки и подарки.'], [], 'channels:connect'); }
  ...
  if (route === 'comments:old') { await saveState(id, { mode: 'await_old_post_forward', section: 'comments' }); return screen('📌 Старый пост', ['Перешлите сюда уже опубликованный пост.', 'Бот аккуратно добавит или восстановит кнопку комментариев без дублей.'], [['📌 Выбрать пост', 'comments:posts']], 'comments:old'); }
  ...
  if (route === 'gifts:test') { const m = missingGiftFields(s); return screen('🧪 Тестовая выдача', m.length ? ['Подарок ещё не готов.', 'Нужно: ' + m.join(', ') + '.'] : ['Подарок готов к тестовой выдаче.', `Название: ${clean(s.giftTitle)}`, `Пост: ${showPost(s)}`], [], 'gifts:home'); }
  ...
  if (route === 'buttons:preview') { const m = missingButtonFields(s); return screen('👀 Предпросмотр кнопки', m.length ? ['Кнопка ещё не готова.', 'Нужно: ' + m.join(', ') + '.'] : ['Так увидит пользователь:', `Кнопка: ${clean(s.ctaButtonText)}`, `Действие: ${clean(s.ctaButtonLink) ? 'открыть ссылку/действие' : s.buttonsRequestContact ? 'запросить контакт' : 'запросить гео'}`, `Пост: ${showPost(s)}`], [], 'buttons:home'); }
  if (route === 'buttons:save') { const m = missingButtonFields(s); if (m.length) return screen('💾 Сохранить кнопку', ['Кнопку пока нельзя сохранить.', 'Нужно: ' + m.join(', ') + '.'], [['✏️ Текст кнопки', 'buttons:input:ctaButtonText'], ['🔗 Ссылка/действие', 'buttons:input:ctaButtonLink'], ['📌 Выбрать пост', 'buttons:posts']], 'buttons:home'); await saveState(id, { buttonsEnabled: true, ctaSavedAt: Date.now() }); return screen('💾 Сохранено', ['Кнопка сохранена для выбранного поста.', `Пост: ${showPost(await getState(id))}`, `Текст: ${clean(s.ctaButtonText)}`], [['👀 Предпросмотр', 'buttons:preview']], 'buttons:home'); }
  ...
  if (route === 'moderation:log') return screen('📖 Журнал модерации', ['Здесь будут последние срабатывания модерации: удалённые комментарии, стоп-слова и причины блокировки.', '', ...moderationStatus(s, ch)], [], 'moderation:home');
