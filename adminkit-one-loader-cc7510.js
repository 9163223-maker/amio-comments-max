...
const SOURCE = 'adminkit-cc7-5-10-button-ux-cleanup';
const MARKER = '__ADMINKIT_CC7_5_10_BUTTON_UX_CLEANUP_LOADER__';
...
function loadLayer(pathName) {
  ...
  catch (error) { item.error = error?.message || String(error); console.warn('[cc7.5.10] layer failed:', pathName, item.error); }
  loadedLayers.push(item);
  return item;
}
function installRoutes(app) {
  if (!app || app.__adminkitCc7510Routes) return app;
  app.__adminkitCc7510Routes = true;
  ...
  app.get('/debug/cc7', (req, res) => {
    ...
    res.json({
      ...
      marker: MARKER,
      installedAt,
      policy: 'button_ux_cleanup_over_759_758_gifts_and_756_comments_core',
      ...
      audit: {
        ...
        giftsCore: 'delegated to CC7.5.8',
        buttonsCore: 'step 4 wording changed to link/where button leads, review edits previous prompt menu after user sends link, stale menu cleanup has retries',
  ...
  app.get(['/debug/ping', '/debug/version', '/debug/build-info'], (req, res) => { noCache(res); res.json({ ok: true, runtimeVersion: RUNTIME, displayVersion: 'CC7.5.10', sourceMarker: SOURCE, buildInfo: readBuildInfo(), loadedLayers, generatedAt: Date.now() }); });
  return app;
}
function installExpressWrap() {
  if (Module.__adminkitCc7510ExpressWrap) return;
  Module.__adminkitCc7510ExpressWrap = true;
  const prev = Module._load;
  Module._load = function adminkitCc7510Load(request, parent, isMain) {
    const loaded = prev.apply(this, arguments);
    if (String(request) === 'express' && loaded && !loaded.__adminkitCc7510Wrapped) {
      function wrappedExpress(...args) { return installRoutes(loaded(...args)); }
      Object.setPrototypeOf(wrappedExpress, loaded);
      Object.assign(wrappedExpress, loaded);
      wrappedExpress.__adminkitCc7510Wrapped = true;
