...
function buildAdminChannelPickerKeyboard(userId = '', source = 'comments') {
  ...
  if (!rows.length) {
    rows.push([{ type: 'callback', text: '📺 Подключить канал', payload: buildAdminCallbackPayload('admin_bind_channel') }]);
  }
...
function buildHelpTextForSection(context = 'main') {
  ...
  if (section === 'comments') {
    return ['Помощь: комментарии', '', '1. Выберите пост или перешлите его боту.', '2. Бот прикрепит кнопку комментариев к посту.', '3. Читатели смогут обсуждать пост в mini-app.', 'Фото и видео открываются обычным тапом; реакции и действия — долгим нажатием.'].join('\n');
  }
