...
## 13. Статус детального дерева подменю

Важное уточнение для следующего чата:

- Верхнее меню из 13 разделов уже согласовано и считается каноном.
- Полное детальное Product Perfect дерево подменю внутри каждого раздела ещё НЕ согласовано как финальный канон.
- По разделам есть продуктовые ожидания, ограничения и частично существующие действия в коде.
- Следующая задача — собрать детальное дерево подменю из live/code, сравнить с целевым продуктовым UX и согласовать.

То есть новый чат не должен придумывать, будто подробное дерево уже полностью утверждено. Он должен построить его из:

1. текущего live/code состояния;
2. канонического файла `features/menu-v3/canonical-menu.js`;
3. целевой 13-раздельной структуры;
4. продуктовых ограничений пользователя.

После этого нужно подготовить Product Perfect audit table по каждому разделу.

---

## 14. Текущий краткий итог
