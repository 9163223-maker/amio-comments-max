...
function rawPayload(c){const v=pv(c);if(!v)return'';return typeof v==='object'?JSON.stringify(v):String(v).trim();}
...
async function ack(config,id,text){if(!id)return null;try{const r=await max.answerCallback({botToken:config.botToken,callbackId:id,notification:text||undefined});pollTrace.add('ack_ok',{callbackId:pollTrace.mask(id),notification:pollTrace.safe(text,80)});return r;}catch(e){pollTrace.add('ack_error',{callbackId:pollTrace.mask(id),error:String(e&&e.message||e),status:e&&e.status});return null;}}
async function show(config,u,c,m,s,edit){const id=mid(m);if(edit&&id){try{const r=await max.editMessage({botToken:config.botToken,messageId:id,text:s.text,attachments:s.attachments,notify:false});pollTrace.add('screen_edit_ok',{screenId:s&&s.id,messageId:pollTrace.mask(id)});return r;}catch(e){pollTrace.add('screen_edit_error_fallback_send',{screenId:s&&s.id,messageId:pollTrace.mask(id),error:String(e&&e.message||e),status:e&&e.status});}}
const chatId=chat(m), userId=uid(u,c,m)||sender(m);const r=await max.sendMessage({botToken:config.botToken,userId:chatId?'':userId,chatId,text:s.text,attachments:s.attachments,notify:false});pollTrace.add('screen_send_ok',{screenId:s&&s.id,chatId:pollTrace.mask(chatId),userId:pollTrace.mask(userId)});return r;}
...
async function handlePollVote({config,c,m,p,userId}){
  const voterId=fallbackVoter(c,m)||userId;
  pollTrace.add('poll_vote_start',{pollId:String(p.pollId||''),optionId:String(p.optionId||''),hasVoterId:!!voterId,callbackId:pollTrace.mask(cbid(c)),messageId:pollTrace.mask(mid(m))});
  const vr=await pollService.vote({userId:voterId,pollId:p.pollId,optionId:p.optionId});
  let patchError='';let key='';
  if(vr&&vr.ok){key=clean(p.commentKey)||clean(vr&&vr.summary&&vr.summary.commentKey);if(key){try{await polls.patchPostWithPoll({config,commentKey:key});}catch(e){patchError=String(e&&e.message||e);}}}
  pollTrace.add('poll_vote_result',{ok:!!(vr&&vr.ok),error:vr&&vr.error||'',pollId:String(p.pollId||''),optionId:String(p.optionId||''),summaryTotal:vr&&vr.summary&&vr.summary.total,commentKeyFound:!!key,patchError});
  await ack(config,cbid(c),vr&&vr.ok?'Голос учтён':'Не удалось учесть голос');
  return{vr,voterId,patchError};
}
function createCleanBot(legacy){const wrapped=base.createCleanBot(legacy);return{handleWebhook:async function(req,res,config){const u=req.body||{},c=cb(u),m=msg(u);try{pollTrace.add('webhook_in',{updateType:String(u.update_type||u.type||''),hasCallback:!!c,hasMessage:!!m,messageId:pollTrace.mask(mid(m)),chatId:pollTrace.mask(chat(m))});if(c){const raw=rawPayload(c);const p=parse(c),a=String(p.action||''),userId=uid(u,c,m)||sender(m);pollTrace.add('callback_received',{action:a||'',raw:pollTrace.safe(raw,160),rawLen:raw.length,callbackId:pollTrace.mask(cbid(c)),userId:pollTrace.mask(userId),pollId:String(p.pollId||''),optionId:String(p.optionId||'')});if(a==='poll_vote'){const out=await handlePollVote({config,c,m,p,userId});return res.status(200).json({ok:true,handledBy:'clean-bot-poll-custom-1542',action:a,voted:!!(out.vr&&out.vr.ok),pollId:p.pollId||'',optionId:p.optionId||'',hasVoterId:!!out.voterId,patchError:out.patchError||'',error:out.vr&&out.vr.error||''});}let s=null;try{s=await pollScreen(p,userId,config);}catch(e){pollTrace.add('poll_screen_error',{action:a,error:String(e&&e.message||e)});s=err(e);}if(s){await ack(config,cbid(c));await show(config,u,c,m,s,true);return res.status(200).json({ok:true,handledBy:'clean-bot-poll-custom-1542',action:a,screenId:s.id});}}
if(m&&text(m).trim()&&!/^\/?start(?:\s|$)/i.test(text(m).trim())){const userId=sender(m)||uid(u,c,m);const s=await polls.handleTextInput(menu,{config,userId,text:text(m)});if(s){pollTrace.add('poll_text_flow',{screenId:s.id,userId:pollTrace.mask(userId),textLen:text(m).length});await show(config,u,c,m,s,false);return res.status(200).json({ok:true,handledBy:'clean-bot-poll-custom-1542',action:'poll_custom_text',screenId:s.id});}}
pollTrace.add('delegate_legacy',{reason:'no_poll_match'});return wrapped.handleWebhook(req,res,config);}catch(e){pollTrace.add('wrapper_error',{error:String(e&&e.message||e),stack:String(e&&e.stack||'').slice(0,500)});if(!res.headersSent)return res.status(500).json({ok:false,error:e&&e.message||'poll_custom_wrapper_failed',handledBy:'clean-bot-poll-custom-1542'});return null;}}};}
