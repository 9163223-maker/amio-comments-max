...
const SOURCE = 'adminkit-cc7-5-18-gift-cleanup-link-content';
const MARKER = '__ADMINKIT_CC7_5_18_GIFT_CLEANUP_LINK_CONTENT_LOADER__';
...
function loadLayer(pathName) {
  ...
  catch (error) { item.error = error?.message || String(error); console.warn('[cc7.5.18] layer failed:', pathName, item.error); }
  loadedLayers.push(item);
  return item;
}
function installRoutes(app) {
  if (!app || app.__adminkitCc7518Routes) return app;
  app.__adminkitCc7518Routes = true;
  ...
  app.get('/debug/cc7', (req, res) => {
    ...
    res.json({ ok: true, runtimeVersion: RUNTIME, sourceMarker: SOURCE, marker: MARKER, installedAt,
      policy: 'gift_cleanup_and_link_content_over_7517_keep_comments_and_buttons_core',
      ...
      adminFlow: (() => { try { return require('./adminkit-admin-flows-7518').selfTest(); } catch (e) { return { ok: false, error: e?.message || String(e) }; } })(),
      ...
      audit: { commentsCore: 'unchanged from accepted CC7.5.6', buttonsCore: 'unchanged from accepted CC7.5.16', giftsCore: 'manager + cleanup/link fix', patcher: 'DB-V3-POST-PATCHER-1.3-MULTI-GIFTS-PRESERVE', noExternalAppButton: true, noOverlay: true },
      commentOpenStateRoute: require('./routes/commentOpenState').selfTest(), generatedAt: Date.now() });
  });
  app.get(['/debug/ping', '/debug/version', '/debug/build-info'], (req, res) => { noCache(res); res.json({ ok: true, runtimeVersion: RUNTIME, displayVersion: 'CC7.5.18', sourceMarker: SOURCE, buildInfo: readBuildInfo(), loadedLayers, generatedAt: Date.now() }); });
  return app;
}
function installExpressWrap() {
  if (Module.__adminkitCc7518ExpressWrap) return;
  Module.__adminkitCc7518ExpressWrap = true;
  const prev = Module._load;
  Module._load = function adminkitCc7518Load(request, parent, isMain) {
    const loaded = prev.apply(this, arguments);
    if (String(request) === 'express' && loaded && !loaded.__adminkitCc7518Wrapped) {
      function wrappedExpress(...args) { return installRoutes(loaded(...args)); }
      Object.setPrototypeOf(wrappedExpress, loaded);
      Object.assign(wrappedExpress, loaded);
      wrappedExpress.__adminkitCc7518Wrapped = true;
