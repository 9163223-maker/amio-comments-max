...
const SOURCE = 'adminkit-cc7-5-9-button-flow-cleanup';
const MARKER = '__ADMINKIT_CC7_5_9_BUTTON_FLOW_CLEANUP_LOADER__';
...
function loadLayer(pathName) {
  ...
  } catch (error) { item.error = error?.message || String(error); console.warn('[cc7.5.9] layer failed:', pathName, item.error); }
  loadedLayers.push(item);
  return item;
}
function installRoutes(app) {
  if (!app || app.__adminkitCc759Routes) return app;
  app.__adminkitCc759Routes = true;
  ...
  app.get('/debug/cc7', (req, res) => {
    ...
    res.json({
      ...
      marker: MARKER,
      installedAt,
      policy: 'button_flow_cleanup_over_758_gifts_and_756_comments_core',
      ...
      audit: {
        ...
        buttonsCore: 'single active menu, stale data reset, save requires text and action, old button menus deleted/retried',
  ...
  app.get(['/debug/ping', '/debug/version', '/debug/build-info'], (req, res) => { noCache(res); res.json({ ok: true, runtimeVersion: RUNTIME, displayVersion: 'CC7.5.9', sourceMarker: SOURCE, buildInfo: readBuildInfo(), loadedLayers, generatedAt: Date.now() }); });
  return app;
}
function installExpressWrap() {
  if (Module.__adminkitCc759ExpressWrap) return;
  Module.__adminkitCc759ExpressWrap = true;
  const prev = Module._load;
  Module._load = function adminkitCc759Load(request, parent, isMain) {
    const loaded = prev.apply(this, arguments);
    if (String(request) === 'express' && loaded && !loaded.__adminkitCc759Wrapped) {
      function wrappedExpress(...args) { return installRoutes(loaded(...args)); }
      Object.setPrototypeOf(wrappedExpress, loaded);
      Object.assign(wrappedExpress, loaded);
      wrappedExpress.__adminkitCc759Wrapped = true;
