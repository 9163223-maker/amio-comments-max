...
const SOURCE = 'adminkit-cc7-5-11-gift-flow-stable';
const MARKER = '__ADMINKIT_CC7_5_11_GIFT_FLOW_STABLE_LOADER__';
...
function fileInfo(relPath, marker) {
  try { const file = path.resolve(__dirname, relPath); const stat = fs.statSync(file); const text = fs.readFileSync(file, 'utf8').slice(0, 2000); return { exists: true, bytes: stat.size, markerFound: marker ? text.includes(marker) : true }; }
...
function loadLayer(pathName) {
  ...
  catch (error) { item.error = error?.message || String(error); console.warn('[cc7.5.11] layer failed:', pathName, item.error); }
  loadedLayers.push(item);
  return item;
}
function installRoutes(app) {
  if (!app || app.__adminkitCc7511Routes) return app;
  app.__adminkitCc7511Routes = true;
  ...
  app.get('/debug/cc7', (req, res) => {
    ...
    res.json({
      ...
      marker: MARKER,
      installedAt,
      policy: 'gift_flow_stable_over_7510_buttons_and_756_comments_core',
      ...
      adminFlow: (() => { try { return require('./adminkit-admin-flows-7511').selfTest(); } catch (e) { return { ok: false, error: e?.message || String(e) }; } })(),
      buildInfo: readBuildInfo(),
      loadedLayers,
      audit: {
        commentsCore: 'unchanged from accepted CC7.5.6',
        buttonsCore: 'kept from CC7.5.10',
        giftsCore: 'CC7.5.11 overrides gift flow only: step 4 content -> step 5 access -> step 6 review/save',
        giftWording: 'no unclear action wording; user sees link/photo/file/text gift content examples',
        noExternalAppButton: true,
        noOverlay: true
      },
  ...
  app.get(['/debug/ping', '/debug/version', '/debug/build-info'], (req, res) => { noCache(res); res.json({ ok: true, runtimeVersion: RUNTIME, displayVersion: 'CC7.5.11', sourceMarker: SOURCE, buildInfo: readBuildInfo(), loadedLayers, generatedAt: Date.now() }); });
  return app;
}
function installExpressWrap() {
  if (Module.__adminkitCc7511ExpressWrap) return;
  Module.__adminkitCc7511ExpressWrap = true;
  const prev = Module._load;
  Module._load = function adminkitCc7511Load(request, parent, isMain) {
    const loaded = prev.apply(this, arguments);
    if (String(request) === 'express' && loaded && !loaded.__adminkitCc7511Wrapped) {
      function wrappedExpress(...args) { return installRoutes(loaded(...args)); }
      Object.setPrototypeOf(wrappedExpress, loaded);
      Object.assign(wrappedExpress, loaded);
      wrappedExpress.__adminkitCc7511Wrapped = true;
