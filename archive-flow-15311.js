...
const RUNTIME = 'ADMINKIT-ARCHIVE-FLOW-1.1-RESTORE-FALLBACK';
let pool = null;
...
async function listArchive({ limit = 8, offset = 0 } = {}) {
  ...
  const result = await query(`SELECT id, channel_id, post_id, comment_key, archive_reason, post_title, post_preview, created_at FROM ak_post_archive ORDER BY created_at DESC, id DESC LIMIT $1 OFFSET $2`, [safeLimit, safeOffset]);
...
async function getArchiveItem(id) {
  ...
  const result = await query(`SELECT id, channel_id, post_id, comment_key, archive_reason, post_title, post_preview, payload, created_at FROM ak_post_archive WHERE id = $1 LIMIT 1`, [archiveId]);
...
async function restoreAsNewPost({ botToken, archiveId }) {
  ...
  try {
    ...
    await query(`INSERT INTO ak_post_archive (channel_id, post_id, comment_key, archive_reason, post_title, post_preview, payload, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7::jsonb, NOW())`, [pack.channelId, restoredMessageId || 'restored-' + Date.now(), clean(item.comment_key), 'restored_as_new_post', clean(item.post_title), clean(item.post_preview), JSON.stringify({ restoredFromArchiveId: item.id, restoredMessageId, maxResponse: sent, original: pack.original })]);
    ...
    await query(`INSERT INTO ak_post_archive (channel_id, post_id, comment_key, archive_reason, post_title, post_preview, payload, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7::jsonb, NOW())`, [pack.channelId, clean(item.post_id || 'restore-failed-' + Date.now()), clean(item.comment_key), 'restore_failed_max_messages', clean(item.post_title), clean(item.post_preview), JSON.stringify({ restoredFromArchiveId: item.id, maxError: details, restorePackage: { channelId: pack.channelId, text: pack.text, attachments: pack.attachments.length } })]);
