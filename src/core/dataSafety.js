...
const RUNTIME = 'ADMINKIT-CORE-DATA-SAFETY-1.1-ACCOUNT-ACCESS';
...
async function ensureCoreStorage() {
  ...
  await safeQuery("create table if not exists ak_accounts (account_id text primary key, plan_code text not null default 'free', status text not null default 'active', features_override jsonb not null default '{}'::jsonb, limits_override jsonb not null default '{}'::jsonb, meta jsonb not null default '{}'::jsonb, created_at timestamptz default now(), updated_at timestamptz default now())");
  await safeQuery("create table if not exists ak_account_admins (admin_id text primary key, account_id text not null, role text not null default 'owner', meta jsonb not null default '{}'::jsonb, created_at timestamptz default now(), updated_at timestamptz default now())");
  await safeQuery("create table if not exists ak_plan_events (id bigserial primary key, account_id text not null, old_plan_code text not null default '', new_plan_code text not null, reason text not null default '', meta jsonb not null default '{}'::jsonb, created_at timestamptz default now())");
  ...
  await addColumnIfMissing('ak_admin_sessions', 'account_id', "text not null default ''");
  await addColumnIfMissing('ak_admin_sessions', 'active_section', "text not null default ''");
  await addColumnIfMissing('ak_admin_sessions', 'active_flow', "text not null default ''");
  await addColumnIfMissing('ak_admin_sessions', 'active_step', "text not null default ''");
  await addColumnIfMissing('ak_admin_sessions', 'selected_channel_id', "text not null default ''");
  await addColumnIfMissing('ak_admin_sessions', 'selected_post_id', "text not null default ''");
  await addColumnIfMissing('ak_admin_sessions', 'selected_comment_key', "text not null default ''");
  await addColumnIfMissing('ak_admin_sessions', 'draft', "jsonb not null default '{}'::jsonb");
  await addColumnIfMissing('ak_admin_sessions', 'active_message_id', "text not null default ''");
  await addColumnIfMissing('ak_admin_sessions', 'garbage_message_ids', "jsonb not null default '[]'::jsonb");
  ...
  await safeQuery('create index if not exists ak_account_admins_account_idx on ak_account_admins(account_id)');
  await safeQuery('create index if not exists ak_plan_events_account_idx on ak_plan_events(account_id, created_at desc)');
  ...
  await markMigration('core-storage-v2-account-access-and-post-addons', 'Core storage exists. Account plans, tariff changes, buttons and lead magnets are preserved with additive migrations only.');
...
function policySummary() {
  return {
    ...
    rules: [
      ...
      'tariff upgrades change access only; they do not rewrite accumulated client content'
    ],
