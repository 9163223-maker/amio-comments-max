...
const RUNTIME = 'ADMINKIT-CORE-STRESS-TEST-1.38.7-REAL-CLICK-PATH';
...
function ctx(route, actor = {}, payload = {}) {
  ...
  return { adminId, admin_id: adminId, userId: adminId, route, updateType: 'stress_test', payload };
}
...
async function cleanupStressData() {
  await db.init();
  const deleted = [];
  for (const sql of ["delete from ak_post_lead_magnets where admin_id='core-stress-admin' or post_id='core-stress-post'", "delete from ak_post_buttons where admin_id='core-stress-admin' or post_id='core-stress-post'", "delete from ak_posts where source='stress_test' or post_id='core-stress-post'", "delete from ak_admin_sessions where admin_id='core-stress-admin'"]) {
    try { const r = await db.query(sql); deleted.push({ sql: sql.split(' where ')[0], rowCount: r.rowCount || 0 }); } catch (e) { deleted.push({ sql: sql.split(' where ')[0], error: e?.message || String(e) }); }
  }
  return deleted;
}
...
async function runLeadClickPath(actor) {
  const home = await routeDispatcher.dispatch(ctx('lead_magnets.home', actor));
  const homeShape = assertScreen(home, 'lead_magnets.home.real_click', true);
  const chBtn = findRouteButton(home, 'lead_magnets.select_channel');
  if (!chBtn) return { skipped: true, reason: 'no_channel_button', homeShape };
  const chPayload = payloadOf(chBtn);
  const posts = await routeDispatcher.dispatch(ctx('lead_magnets.select_channel', actor, chPayload));
  const postsShape = assertScreen(posts, 'lead_magnets.select_channel.real_click', true);
  const postBtn = findRouteButton(posts, 'lead_magnets.post');
  if (!postBtn) return { skipped: true, reason: 'no_post_button_after_channel_select', homeShape, postsShape, chPayload };
  const postPayload = payloadOf(postBtn);
  const post = await routeDispatcher.dispatch(ctx('lead_magnets.post', actor, postPayload));
  const postShape = assertScreen(post, 'lead_magnets.post.real_click', true);
  const addBtn = findRouteButton(post, 'lead_magnets.add');
  let addShape = null;
  if (addBtn) addShape = assertScreen(await routeDispatcher.dispatch(ctx('lead_magnets.add', actor, payloadOf(addBtn))), 'lead_magnets.add.real_click', true);
...
async function run(options = {}) {
  ...
  for (const s of sectionRegistry.listAll()) {
    const route = s.routes?.home || s.id + '.home';
    tests.push(await step('dispatch.section.' + s.id, async () => dispatchAssert(route, actor, { sectionId: s.id }, true)));
  }
...
function selfTest() { return { ok: true, runtimeVersion: RUNTIME, catchesUiDiagnosticScreens: true, realLeadMagnetsClickPath: true }; }
