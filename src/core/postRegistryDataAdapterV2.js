...
const RUNTIME = 'ADMINKIT-CORE-POST-REGISTRY-DATA-ADAPTER-1.38.4-LEGACY-COMMENT-KEY';
...
async function ensure() {
  await db.init();
  await db.query("create table if not exists ak_posts (id bigserial primary key, admin_id text not null default '', channel_id text not null default '', post_id text not null default '', comment_key text not null default '', source text not null default 'manual', meta jsonb not null default '{}'::jsonb, created_at timestamptz default now(), updated_at timestamptz default now())");
  for (const sql of [
    ...
    "update ak_posts set comment_key='' where comment_key is null",
    "alter table ak_posts alter column comment_key set default ''",
...
async function listPosts(ctx = {}, options = {}) {
  ...
  const { rows } = await db.query("select channel_id, channel_title, post_id, comment_key, post_title, post_preview, source, meta, updated_at from ak_posts where ($1='' or admin_id=$1) and ($2='' or channel_id=$2) order by updated_at desc, id desc limit $3", [adminId, channelId, limit]);
  return rows.map((row) => ({ channelId: row.channel_id, channelTitle: humanTitle(row.channel_title) || 'Канал', postId: row.post_id, commentKey: row.comment_key || '', postTitle: humanTitle(row.post_title, row.post_preview) || 'Пост', displayTitle: humanTitle(row.post_title, row.post_preview) || ('Пост ' + row.post_id), postPreview: clean(row.post_preview), source: row.source, meta: row.meta || {}, updatedAt: row.updated_at }));
}
...
async function upsertPost(ctx = {}, input = {}) {
  ...
  const sql = "insert into ak_posts(admin_id,channel_id,channel_title,post_id,comment_key,post_title,post_preview,source,meta,updated_at) values($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,now()) on conflict(channel_id,post_id) do update set admin_id=excluded.admin_id, channel_title=coalesce(nullif(excluded.channel_title,''),ak_posts.channel_title), comment_key=coalesce(nullif(excluded.comment_key,''),ak_posts.comment_key), post_title=coalesce(nullif(excluded.post_title,''),ak_posts.post_title), post_preview=coalesce(nullif(excluded.post_preview,''),ak_posts.post_preview), source=excluded.source, meta=ak_posts.meta || excluded.meta, updated_at=now() returning channel_id, channel_title, post_id, comment_key, post_title, post_preview, source, meta";
  const { rows } = await db.query(sql, [adminId, channelId, channelTitle, postId, commentKey, postTitle, postPreview, source, meta]);
  const row = rows[0];
  return { ok: true, post: { channelId: row.channel_id, channelTitle: humanTitle(row.channel_title), postId: row.post_id, commentKey: row.comment_key || '', postTitle: humanTitle(row.post_title, row.post_preview), postPreview: row.post_preview, source: row.source, meta: row.meta || {} } };
}
...
async function captureForwardedPost(ctx = {}) {
  ...
  return upsertPost(ctx, { postId, commentKey: payload.commentKey || ctx.commentKey || '', postTitle: payload.postTitle || ctx.text || 'Пересланный пост', postPreview: ctx.text || payload.postPreview || '', source: 'forwarded_post', meta: { capturedAt: new Date().toISOString(), rawPayloadKeys: Object.keys(payload) } });
}

function selfTest() { return { ok: true, runtimeVersion: RUNTIME, table: 'ak_posts', schemaMigrationReady: true, legacyCommentKeyCompatible: true, listChannelsReady: true, listPostsReady: true, upsertPostReady: true, captureForwardedPostReady: true, humanTitlesReady: true, noLegacyAdapters: true }; }
