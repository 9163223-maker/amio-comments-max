...
const RUNTIME = 'ADMINKIT-CORE-ROUTE-DISPATCHER-1.4-POST-CAPTURE-FLOW';
...
function flowErrorScreen(selected, ctx = {}) {
  ...
  if (error === 'stale_flow_callback') return menuRenderer.renderScreen({ title: '⚠️ Старое меню больше не активно', body: ['Эта кнопка относится к другому сценарию и не будет выполнена.', selected.expectedFlow ? `Активный сценарий: ${selected.expectedFlow}` : '', selected.actualFlow ? `Кнопка из сценария: ${selected.actualFlow}` : '', 'Начните действие заново из нужного раздела.'].filter(Boolean), buttons: [{ text: '↩️ Назад к разделу', route: backRouteForFlow(selected.expectedFlow || ctx.payload?.flowId || selected.flow?.id || '') }], homeRoute: 'main.home' });
...
function saveLeadSuccessScreen(saved = {}) {
  ...
  return menuRenderer.renderScreen({ title: '✅ Лид-магнит сохранён в Core', body: [`Пост: ${postTitle}`, channelTitle ? `Канал: ${channelTitle}` : '', `Название: ${gift.title || saved.title || ''}`, saved.materialPreview ? `Материал: ${saved.materialPreview}` : '', saved.accessLabel ? `Условия: ${saved.accessLabel}` : '', saved.conditionVerifier ? `Проверка: ${saved.conditionVerifier}` : '', '', 'Сохранено в чистую таблицу: ak_post_lead_magnets.', 'Legacy adapters не использовались.', 'Выдачу подарка в личные сообщения подключим отдельным clean delivery-flow.'].filter(Boolean), buttons: [{ text: '🎁 К лид-магнитам', route: 'lead_magnets.home', data: { sectionId: 'lead_magnets', refresh: 1 } }], homeRoute: 'main.home' });
}
async function saveButtonFromFlow(ctx = {}) {
  const current = await flowEngine.getCurrent(ctx); if (!current.ok) return flowErrorScreen(current, ctx);
  ...
  const draft = current.draft || {}; const saveCtx = { ...ctx, adminId: ctx.adminId || ctx.admin_id || current.session?.admin_id || '', channelId: draft.channelId || current.session?.selected_channel_id || ctx.channelId || ctx.payload?.channelId || '', channelTitle: draft.channelTitle || ctx.channelTitle || ctx.payload?.channelTitle || '', postId: draft.postId || current.session?.selected_post_id || ctx.postId || ctx.payload?.postId || '', postTitle: draft.postTitle || ctx.postTitle || ctx.payload?.postTitle || '', session: current.session, draft };
  ...
  await flowEngine.cancel(ctx, 'button_saved_core_clean'); return saveButtonSuccessScreen({ ...saved, postId: saveCtx.postId, postTitle: saveCtx.postTitle, channelTitle: saveCtx.channelTitle, ctx: saveCtx });
}
async function saveLeadMagnetFromFlow(ctx = {}) {
  ...
  const saved = await postAddonManager.addLeadMagnet(saveCtx, { title: draft.leadMagnetTitle || draft.title || '', materialType, text: materialType === 'text' ? (draft.material || '') : '', url: materialType === 'url' ? (draft.material || '') : '', accessMode: draft.accessMode || 'subscribers_current_channel', conditions: draft.conditions || {}, meta: { source: 'adminkit-core', flowId: 'lead_magnets.create', conditionVerifier: draft.conditionVerifier || '', savedAt: new Date().toISOString(), legacyAdaptersUsed: false } });
  if (!saved.ok) return flowErrorScreen({ ...saved, flow: current.flow, step: current.step, draft }, ctx);
  await flowEngine.cancel(ctx, 'lead_magnet_saved_core_clean'); return saveLeadSuccessScreen({ ...saved, title: draft.leadMagnetTitle || draft.title || '', materialPreview: draft.materialPreview || '', accessLabel: draft.accessLabel || '', conditionVerifier: draft.conditionVerifier || '', postId: saveCtx.postId, postTitle: saveCtx.postTitle, channelTitle: saveCtx.channelTitle, ctx: saveCtx });
}

async function dispatch(ctx = {}) {
  const route = String(ctx.route || '').trim(); const normalizedRoute = route.toLowerCase();
  ...
  if (route === 'flow.capture_post') { const captured = await flowEngine.capturePost(ctx); if (!captured.ok) return flowErrorScreen(captured, ctx); return renderFlow(captured); }
  ...
  if (route === 'flow.save') { const current = await flowEngine.getCurrent(ctx); if (current.flow?.id === 'lead_magnets.create') return saveLeadMagnetFromFlow(ctx); return saveButtonFromFlow(ctx); }
  ...
  const map = sectionRegistry.routeMap(); const section = map.get(route) || sectionRegistry.find(String(ctx.payload?.sectionId || ''));
