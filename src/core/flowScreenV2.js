...
const RUNTIME = 'ADMINKIT-CORE-FLOW-SCREEN-1.39.0-USER-FRIENDLY';
...
function stepHint(flowId, stepId) {
  if (flowId === 'lead_magnets.create') {
    ...
    if (stepId === 'condition_setup') return 'Настройте параметры условия.';
...
function channelButtons(flowId, channels = []) {
  ...
  return channels.slice(0, 10).map((channel, index) => {
    const title = human(channel.channelTitle || channel.title || channel.displayTitle, 'Текущий канал');
    const count = Number(channel.postCount || channel.postsCount || 0);
    return { text: `${index + 1}. ${cut(title, 42)}${count ? ` · ${count} постов` : ''}`, route: 'flow.select_channel', data: { flowId, channelId: String(channel.channelId || channel.id || ''), channelTitle: cut(title, 100) } };
  });
}
...
function postPickerButtons(flowId, posts = []) {
  ...
  if (posts.length) {
    buttons.push(...posts.slice(0, 8).map((post, index) => {
      const label = human(post.displayTitle || post.title || post.postTitle || post.postPreview, 'Пост без текста');
      return { text: `${index + 1}. ${cut(label, 55)}`, route: 'flow.select_post', data: { flowId, postId: String(post.postId || post.id || post.key || ''), postTitle: cut(label, 100), ...(post.channelId ? { channelId: String(post.channelId) } : {}), ...(post.channelTitle ? { channelTitle: cut(human(post.channelTitle, 'Текущий канал'), 100) } : {}) } };
    }));
  } else {
...
function renderFlowState(result = {}, options = {}) {
  ...
  if (step.id === 'condition_setup') buttons.push(...conditionSetupButtons(flow.id, draft));
...
function selfTest() { const catalog = conditionCatalog.selfTest(); return { ok: true, runtimeVersion: RUNTIME, userFriendly: true, noFlowStepDebugInUserScreens: true, noRawPostIdsInButtons: true, leadConditionCatalogReady: catalog.ok === true, leadConditionCount: catalog.count }; }
