...
const RUNTIME = 'ADMINKIT-CORE-STRESS-TEST-1.50.0-DEBUG-GITHUB-EXPORT';
...
function ctx(route, payload = {}) { return { adminId: STRESS_ADMIN_ID, admin_id: STRESS_ADMIN_ID, userId: STRESS_ADMIN_ID, route, planCode: 'start', updateType: 'stress_test_debug_export_1500', payload: { ...(payload || {}) } }; }
...
async function runDebugExportScenario() {
  ...
  assertRoute(main.screen, 'debug_diagnostics.home', 'debug.main.visible');
  ...
  return {
    ...
    checks: [
      'debug_section_registered_and_visible',
...
async function runFast(options = {}) {
  ...
  return {
    ...
    mode: 'fast_compact_scenario_1500_debug_github_export',
    summary: {
      ...(baseResult.summary || {}),
    ...
    status: failed.length ? 'FAILED — см. failed' : 'OK — Core 1.50.0: Debug / GitHub export прошёл сценарный обход',
    failed,
    slow,
    ...
    notes: [
      ...
      'Core 1.50.0 наполняет раздел Debug / диагностика финальным деревом функций.',
      ...
      'GitHub export имеет dry-run, latest.json и latest-lite.json.',
      'Токены не выводятся в интерфейс и редактируются в payload.'
    ]
  };
}
