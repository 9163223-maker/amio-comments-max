...
const RUNTIME = 'ADMINKIT-CORE-POST-REGISTRY-DATA-ADAPTER-1.35';
...
function humanTitle(...values) {
  for (const value of values) {
    const s = clean(value);
    if (s && !isRawId(s)) return s.slice(0, 120);
  }
  return '';
}
...
async function ensure() {
  await db.init();
  await db.query("create table if not exists ak_posts (id bigserial primary key, admin_id text not null default '', channel_id text not null default '', channel_title text not null default '', post_id text not null default '', post_title text not null default '', post_preview text not null default '', source text not null default 'manual', meta jsonb not null default '{}'::jsonb, created_at timestamptz default now(), updated_at timestamptz default now(), unique(channel_id, post_id))");
}
...
function selfTest() {
  return { ok: true, runtimeVersion: RUNTIME, table: 'ak_posts', listChannelsReady: true, listPostsReady: true, upsertPostReady: true, captureForwardedPostReady: true, humanTitlesReady: true, noLegacyAdapters: true };
}
