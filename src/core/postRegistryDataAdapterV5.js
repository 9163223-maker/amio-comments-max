...
const RUNTIME = 'ADMINKIT-CORE-POST-REGISTRY-DATA-ADAPTER-1.38.9-SAFE-HUMAN-LABELS';
...
function metaText(meta = {}) {
  ...
  for (const key of ['post', 'message', 'original', 'raw']) {
    const nested = m[key];
    if (nested && typeof nested === 'object') {
      const found = humanTitle(nested.title, nested.text, nested.caption, nested.preview, nested.body, nested.content, nested.messageText, nested.message_text);
      if (found) return found;
    }
  }
  return '';
}
function postDisplay(row = {}) { return humanTitle(row.post_title, row.post_preview, metaText(row.meta)) || 'Пост без текста'; }
function channelDisplay(row = {}) { return humanTitle(row.channel_title, row.title, row.channel_name, row.display_title) || 'Текущий канал'; }
...
async function listPosts(ctx = {}, options = {}) {
  ...
  return (rows || []).map((row) => {
    ...
    return { channelId: clean(row.channel_id), channelTitle: channelDisplay(row), postId: clean(row.post_id), commentKey: clean(row.comment_key), postTitle: display, displayTitle: display, postPreview: preview, source: row.source, meta: row.meta || {}, updatedAt: row.updated_at };
  });
}

function selfTest() { return { ...base.selfTest(), ok: true, runtimeVersion: RUNTIME, safeChannelTitleJoin: true, userFriendlyPostPreviews: true, noRawPostIdsInLabels: true, noTechnicalIdsInLabels: true }; }
