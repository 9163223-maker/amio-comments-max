...
const SOURCE = 'adminkit-cc7-5-22-lead-conditions-save-stable';
const MARKER = '__ADMINKIT_CC7_5_22_LEAD_CONDITIONS_SAVE_STABLE_LOADER__';
...
function installRoutes(app) {
  ...
  app.get('/debug/cc7', (req, res) => {
    ...
    res.json({ ok: true, runtimeVersion: RUNTIME, sourceMarker: SOURCE, marker: MARKER, installedAt,
      policy: 'active_lead_magnet_conditions_with_save_persistence_over_7520',
      ...
      adminFlow: (() => { try { return require('./adminkit-admin-flows-7522').selfTest(); } catch (e) { return { ok: false, error: e?.message || String(e) }; } })(),
      ...
      audit: { commentsCore: 'unchanged from accepted CC7.5.6', buttonsCore: 'unchanged from accepted CC7.5.16+', leadMagnetsCore: 'active conditions wrapper over CC7.5.20 with post-save persistence', activeConditions: ['all', 'subscribers_current_channel', 'comments_min', 'comment_keyword', 'channels_many'], plannedConditions: ['referral'], noExternalAppButton: true, noOverlay: true, optimizationNote: 'still layered; after acceptance merge admin-flow wrappers into one clean core' },
      commentOpenStateRoute: require('./routes/commentOpenState').selfTest(), generatedAt: Date.now() });
  });
  app.get(['/debug/ping', '/debug/version', '/debug/build-info'], (req, res) => { noCache(res); res.json({ ok: true, runtimeVersion: RUNTIME, displayVersion: 'CC7.5.22', sourceMarker: SOURCE, buildInfo: readBuildInfo(), loadedLayers, generatedAt: Date.now() }); });
  return app;
}
function installExpressWrap() { if (Module.__adminkitCc7522ExpressWrap) return; Module.__adminkitCc7522ExpressWrap = true; const prev = Module._load; Module._load = function adminkitCc7522Load(request, parent, isMain) { const loaded = prev.apply(this, arguments); if (String(request) === 'express' && loaded && !loaded.__adminkitCc7522Wrapped) { function wrappedExpress(...args) { return installRoutes(loaded(...args)); } Object.setPrototypeOf(wrappedExpress, loaded); Object.assign(wrappedExpress, loaded); wrappedExpress.__adminkitCc7522Wrapped = true; return wrappedExpress; } return loaded; }; }
