...
function postTitle(target = null) { return short(target?.originalText || target?.postText || target?.text || target?.caption || target?.postId || 'выбранный пост', 70); }
...
function patchLine(result = {}) { if (result.ok) { if (result.skipped && result.reason === 'already_patched') return 'Кнопка под постом: уже была актуальна.'; return 'Кнопка под постом: добавлена/обновлена.'; } const reason = clean(result.reason || result.error?.message || 'patch_failed'); return `Подарок сохранён, но кнопку под постом обновить не удалось: ${reason}`; }
...
function rewriteScreen(screen = null, ctx = {}) { if (!screen) return screen; const id = clean(screen.id); let text = String(screen.text || ''); if (/start_create|gift_start/i.test(id) || /foundation-PR|Post ID:|post_edit_window_expire/i.test(text)) { const target = targetFromState(ctx.userId); return { ...screen, id: 'adminkit_gift_step_1_material', text: ['🎁 Создание подарка', '', 'Шаг 1 — материал подарка', '', 'Пришлите ссылку на подарок.', '', `Пост выбран: ${postTitle(target)}`, '', 'Условия получения настроим дальше.'].join('\n') }; }
