...
// CC7.5.3 comments open-state first checkpoint.
// Keeps CC7.5.2 proven loader chain and pairs it with public/app-onepass.js CC7.5.3.
// Fix focus: mini-app should try comment-open-state before showing Start landing.
...
const RUNTIME = 'CC7.5.3-OPENSTATE-FIRST-COMMENTS-FIX';
const SOURCE = 'adminkit-cc7-5-3-openstate-first-comments-fix';
const MARKER = '__ADMINKIT_CC7_5_3_OPENSTATE_FIRST_COMMENTS_FIX__';
...
function loadLayer(pathName) {
  ...
  try {
    ...
    console.warn('[cc7.5.3] layer failed:', pathName, item.error);
  }
...
function installAdminFlowPatch() {
  try {
    ...
    if (postPatcher.__adminkitCc753Patched || postPatcher.__adminkitCc752Patched || postPatcher.__adminkitCc747Patched) {
    ...
    postPatcher.patchStoredPost = async function adminkitCc753PatchStoredPost(args = {}) {
      if (args && args.commentKey) markDirty(args.commentKey, 'cc753_before_patchStoredPost');
      return originalPatchStoredPost.call(this, args);
    };
    postPatcher.__adminkitCc753Patched = true;
    adminFlowPatch = {
      ok: true,
    ...
    console.warn('[cc7.5.3] admin flow patch failed:', adminFlowPatch.error);
    return adminFlowPatch;
  }
}
...
function appOnepassInfo() {
  try {
    ...
    const text = fs.readFileSync(file, 'utf8').slice(0, 240);
    return { exists: true, bytes: stat.size, servedByStatic: true, markerFound: text.includes('CC7.5.3-APPJS-OPENSTATE-FIRST') };
  } catch (error) {
...
function installRoutes(app) {
  if (!app || app.__adminkitCc753Routes) return app;
  app.__adminkitCc753Routes = true;
  ...
  app.get('/debug/cc7', (req, res) => {
    ...
    res.json({
      ...
      marker: MARKER,
      installedAt,
      policy: 'openstate_first_comments_fix_no_archive_lite_no_clean_menu_core',
      ...
      loadedLayers,
      audit: {
        base: 'CC7.5.2 proven loader chain',
        appFix: 'public/app-onepass.js now asks /api/adminkit/comment-open-state before showing landing',
        removedForThisCheckpoint: ['adminkit-archive-lite-layer', 'adminkit-clean-menu-core'],
        commentsCore: 'kept: routes/commentOpenState.js + services/postMetaService.js + services/maxApi.js native open_app payload path',
...
function installExpressWrap() {
  if (Module.__adminkitCc753ExpressWrap) return;
  Module.__adminkitCc753ExpressWrap = true;
  const prev = Module._load;
  Module._load = function adminkitCc753Load(request, parent, isMain) {
    const loaded = prev.apply(this, arguments);
    try {
      if (String(request) === 'express' && loaded && !loaded.__adminkitCc753Wrapped) {
        ...
        wrappedExpress.__adminkitCc753Wrapped = true;
        return wrappedExpress;
      }
    } catch (error) {
      console.warn('[cc7.5.3] express wrap failed:', error?.message || error);
    }
    return loaded;
  };
}
...
function layerSummary() {
  ...
  return {
    ...
    useOpenAppButton: process.env.ADMINKIT_USE_OPEN_APP_BUTTON,
    policy: 'openstate_first_comments_fix_no_ui_redesign'
  };
}
