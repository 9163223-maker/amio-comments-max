...
const RUNTIME = 'CC6.5.9.3-SAFE-COMMENTS-LAUNCHER';
const SOURCE = 'adminkit-CC6.5.9.3-isolated-comments-openapp-launcher';
const MARKER = '__ADMINKIT_COMMENTS_SAFE_LAUNCHER_593__';
let installed = false;
  ...
  fs.readFileSync = function patchedRead(filePath, options) {
    ...
    try {
      ...
      if (path.resolve(String(filePath || '')) === appPath && wantsText) {
        ...
        if (!text.includes('__ADMINKIT_COMMENTS_SAFE_LAUNCHER_593__')) {
...
function patchExpress() {
  if (Module.__adminkitCommentsSafeLauncher593ExpressPatched) return;
  Module.__adminkitCommentsSafeLauncher593ExpressPatched = true;
  const oldLoad = Module._load;
  Module._load = function patchedLoad(request, parent, isMain) {
    const loaded = oldLoad.apply(this, arguments);
    try {
      if (String(request || '') === 'express' && loaded && !loaded.__adminkitCommentsSafeLauncher593Wrapped) {
        function wrappedExpress(...args) {
          const app = loaded(...args);
          if (app && !app.__adminkitCommentsSafeLauncher593Route) {
            app.__adminkitCommentsSafeLauncher593Route = true;
        ...
        wrappedExpress.__adminkitCommentsSafeLauncher593Wrapped = true;
