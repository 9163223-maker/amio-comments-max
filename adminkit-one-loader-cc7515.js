...
const SOURCE = 'adminkit-cc7-5-15-button-manager-ux-summary';
const MARKER = '__ADMINKIT_CC7_5_15_BUTTON_MANAGER_UX_LOADER__';
...
function loadLayer(pathName) {
  ...
  catch (error) { item.error = error?.message || String(error); console.warn('[cc7.5.15] layer failed:', pathName, item.error); }
  loadedLayers.push(item);
  return item;
}
function installRoutes(app) {
  if (!app || app.__adminkitCc7515Routes) return app;
  app.__adminkitCc7515Routes = true;
  ...
  app.get('/debug/cc7', (req, res) => {
    ...
    res.json({ ok: true, runtimeVersion: RUNTIME, sourceMarker: SOURCE, marker: MARKER, installedAt,
      policy: 'button_manager_ux_summary_over_7514_keep_comments_gifts_addons',
      ...
      adminFlow: (() => { try { return require('./adminkit-admin-flows-7515').selfTest(); } catch (e) { return { ok: false, error: e?.message || String(e) }; } })(),
      ...
      audit: { commentsCore: 'unchanged from accepted CC7.5.6', giftsCore: 'unchanged from CC7.5.12/7.5.14', buttonsCore: 'UX polish only: final summary shows all buttons and manager keyboard is single-column', noExternalAppButton: true, noOverlay: true, optimizationNote: 'still layered; after acceptance merge admin flow wrappers 7510-7515 into one clean core module' },
      commentOpenStateRoute: require('./routes/commentOpenState').selfTest(), generatedAt: Date.now() });
  });
  app.get(['/debug/ping', '/debug/version', '/debug/build-info'], (req, res) => { noCache(res); res.json({ ok: true, runtimeVersion: RUNTIME, displayVersion: 'CC7.5.15', sourceMarker: SOURCE, buildInfo: readBuildInfo(), loadedLayers, generatedAt: Date.now() }); });
  return app;
}
function installExpressWrap() {
  if (Module.__adminkitCc7515ExpressWrap) return;
  Module.__adminkitCc7515ExpressWrap = true;
  const prev = Module._load;
  Module._load = function adminkitCc7515Load(request, parent, isMain) {
    const loaded = prev.apply(this, arguments);
    if (String(request) === 'express' && loaded && !loaded.__adminkitCc7515Wrapped) {
      function wrappedExpress(...args) { return installRoutes(loaded(...args)); }
      Object.setPrototypeOf(wrappedExpress, loaded);
      Object.assign(wrappedExpress, loaded);
      wrappedExpress.__adminkitCc7515Wrapped = true;
