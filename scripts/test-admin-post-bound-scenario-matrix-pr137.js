...
async function testGifts() {
  ...
  const liveHome = await scenario('gifts', 'Gifts live/canonical entry', 'gifts:home', (s) => /(^|_)gifts_clean_home$/.test(s.id) && /Создать подарок/.test(visible(s)) && /Заменить подарок/.test(visible(s)) && /Текущий подарок/.test(visible(s)) && /Список подарков/.test(visible(s)) && /Главное меню/.test(visible(s)) && !/Подарок под постом|Шаг 1|материал подарка|commentKey|postId|channelId|token|payload|trace/i.test(visible(s)), () => call(gifts, { action: 'gifts:home' }));
  assertNoRawOrInternal(liveHome, 'gifts live/canonical entry');
  const cleanPrimary = labels(home).filter((label) => /Создать подарок|Заменить подарок|Главное меню/.test(label)).map((label) => label.replace(/^[^А-ЯA-Z]+\s*/i, '').replace(/ к выбранному посту$/, '')).sort();
  const livePrimary = labels(liveHome).filter((label) => /Создать подарок|Заменить подарок|Главное меню/.test(label)).map((label) => label.replace(/^[^А-ЯA-Z]+\s*/i, '').replace(/ к выбранному посту$/, '')).sort();
  assert.deepStrictEqual(livePrimary, cleanPrimary, 'gifts clean entry parity: gifts:home exposes same primary actions as admin_section_gifts');
  setOnlySetup({ giftTargetPost: rawTarget(KEY_SELFTEST), buttonTargetPost: rawTarget(KEY2), commentTargetPost: rawTarget(KEY2), giftsCurrentCard: null, giftFlow: null });
  const guardedLiveHome = await scenario('gifts', 'Gifts live/canonical entry stale targets', 'gifts:home', (s) => /(^|_)gifts_clean_home$/.test(s.id) && /Заменить подарок/.test(visible(s)) && !/selftest|Матрица: выбранный|Шаг 1|материал подарка/i.test(visible(s)), () => call(gifts, { action: 'gifts:home' }));
  assertNoRawOrInternal(guardedLiveHome, 'gifts live/canonical stale targets');
  ...
  const liveStaleHome = await call(gifts, { action: 'gifts:home' });
  const liveReplacePicker = await scenario('gifts', 'Replace from live/canonical entry', 'gifts:home → Заменить подарок', (s) => /gifts_clean_channel_picker$/.test(s.id) && !/Матрица: выбранный|материал подарка/i.test(visible(s)), () => call(gifts, payloadFor(liveStaleHome, /Заменить подарок/)));
  assertPickerSafety(liveReplacePicker, 'gifts replace from live/canonical entry');
  setOnlySetup({ giftTargetPost: null, giftsCurrentCard: null, giftFlow: null, commentTargetPost: null, buttonTargetPost: null, activeAdminFlowKind: '' });
