...
(async () => {
  ...
  assert.strictEqual(pkg.buildVersion, 'CC8.3.58-PR192-PUSH-DEVICE-AUTOCONNECT-UNSUBSCRIBE', 'runtime marker advances to PR173');
  assert.strictEqual(pkg.sourceMarker, 'adminkit-pr192-push-device-autoconnect-unsubscribe', 'source marker advances to PR173');
  assert(entrypoint.includes("const RUNTIME='CC8.3.58-PR192-PUSH-DEVICE-AUTOCONNECT-UNSUBSCRIBE'"), 'active entrypoint has PR173 runtime marker');
  assert(entrypoint.includes("const SOURCE='adminkit-pr192-push-device-autoconnect-unsubscribe'"), 'active entrypoint has PR173 source marker');
