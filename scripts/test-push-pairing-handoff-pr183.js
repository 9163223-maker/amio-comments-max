...
(async () => {
  ...
  try {
    ...
    try {
      ...
      assert(!joinB.text.includes('existingActiveDevicesFound'), 'join page does not infer devices from maxUserId');
      assert(joinB.text.includes('Подключается чат:') && joinB.text.includes('«Chat B»'), 'Safari page names the discovered chat without JavaScript');
      assert(joinB.text.includes('Откройте АдминКИТ PUSH с экрана Домой.'), 'Safari page keeps the PWA fallback instruction');
      ...
      events.length = 0;
      const pendingB = await request(server, '/api/push/pending', postJson({ subscription: deviceSubscription }, { cookie: `push_pairing_handoff=${encodeURIComponent(handoffB)}` }));
      assert.strictEqual(pendingB.status, 200);
      assert.strictEqual(pendingB.body.pending[0].handoffId, handoffB, 'same-device handoff cookie recovers the pending handoff');
