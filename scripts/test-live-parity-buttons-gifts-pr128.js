...
function assertNoRaw(text, label) { assert.ok(!/selftest_comments_matrix_channel|debug|legacy|selftest/i.test(text), `${label}: no selftest/debug/legacy`); assert.ok(!/-753\d{6}|\b(?:channelId|postId|messageId|commentKey)\b/i.test(text), `${label}: no raw ids or key names`); }
...
async function main() {
  ...
  try {
    ...
    assert.strictEqual(last.ok, true, 'debug ui-last returns last replay diagnostics');
  } finally {
