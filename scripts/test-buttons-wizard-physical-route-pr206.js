...
(async () => {
  ...
  assert.strictEqual(probe.variants.saveInFlight.ok, true, 'save-in-flight screen edits original preview without fresh duplicate message');
  assert.strictEqual(probe.variants.saveInFlight.freshSendCount, 0, 'save-in-flight screen is not sent fresh');
  assert.strictEqual(probe.variants.saveInFlight.previewEdited, true, 'save-in-flight disables the original preview');
  assert.strictEqual(probe.variants.plain.repeatSaveIdempotentOk, true, 'repeated save on current preview returns visible idempotent result');
  assert(probe.variants.plain.repeatSaveNewSends >= 1, 'repeated save creates a fresh visible idempotent result');
