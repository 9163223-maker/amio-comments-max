...
assert.deepStrictEqual(canonical.clientSections.map((section) => section.title), expectedSections, 'client top-level sections must preserve approved order with PR171 Push entry');
...
for (const section of canonical.clientSections) {
  ...
  assertHasAll(rootLabels, section.id === 'push_notifications' ? ['Как это работает', 'Главное меню'] : ['❓ Помощь по разделу', '🏠 Главное меню'], `${section.id} root navigation`);
