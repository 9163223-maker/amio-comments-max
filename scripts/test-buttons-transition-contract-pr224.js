...
(async () => {
  ...
  try {
    ...
    reset(); patchCalls.length = 0; post('legacy-delete', 'ch-a:2', { importedButtons: [{ id: 'legacy_del', text: 'Legacy delete', url: 'https://legacy-delete.example' }] }); const legacyDeleteSelected = await select('legacy-delete', 'ch-a:2');
    assert(/Legacy delete → https:\/\/legacy-delete\.example/.test(txt(legacyDeleteSelected)), 'BTN-062 imported button shown'); assert.strictEqual(buttons.reconcileButtonsFeatureState({ userId: 'legacy-delete', target: { commentKey: 'ch-a:2', channelId: 'ch-a', postId: '2' } }).source, 'canonical_button_set', 'BTN-062 imported into canonical');
    const legacyDeleteConfirm = await route('legacy-delete', 'button_admin_delete_confirm', btn(legacyDeleteSelected, 'button_admin_delete_confirm').payload); const legacyDeleted = await route('legacy-delete', 'button_admin_delete', btn(legacyDeleteConfirm, 'button_admin_delete').payload);
    assert(/Текущие кнопки: пока нет кнопок/.test(txt(legacyDeleted)), 'BTN-062 deleted legacy button stays empty'); assert(!labels(legacyDeleted).includes('✏️ Изменить кнопку') && !labels(legacyDeleted).includes('🗑 Удалить кнопку'), 'BTN-062 hides edit/delete'); assertCommit(last('button_admin_delete'), 0, /delete/);
    const postDeleteReconcile = buttons.reconcileButtonsFeatureState({ userId: 'legacy-delete', target: { commentKey: 'ch-a:2', channelId: 'ch-a', postId: '2' } }); assert.strictEqual(postDeleteReconcile.buttons.length, 0, 'BTN-062 reconcile does not resurrect deleted legacy'); assert.strictEqual(postDeleteReconcile.source, 'empty', 'BTN-062 reimport blocked source empty');

    const reopenedRoot = await route('legacy-delete', 'admin_section_buttons'); const reopenedCurrent = await route('legacy-delete', 'button_admin_show_current'); const reselected = await select('legacy-delete', 'ch-a:2');
    for (const screen of [reopenedRoot, reopenedCurrent, reselected]) { assert(/Текущие кнопки: пока нет кнопок/.test(txt(screen)), 'BTN-063 reopen remains empty'); assert(!labels(screen).includes('✏️ Изменить кнопку') && !labels(screen).includes('🗑 Удалить кнопку'), 'BTN-063 no edit/delete after reopen'); assert(!txt(screen).includes('Legacy delete'), 'BTN-063 old legacy button not shown'); }

    reset(); post('legacy-fresh', 'ch-a:2', { legacyButtons: [{ id: 'legacy_fresh', text: 'Legacy fresh', url: 'https://legacy-fresh.example' }] }); const freshLegacy = await select('legacy-fresh', 'ch-a:2');
    assert(/Legacy fresh → https:\/\/legacy-fresh\.example/.test(txt(freshLegacy)), 'BTN-064 fresh legacy imports'); await executable('legacy-fresh', freshLegacy, 'button_admin_edit', 'Кнопка: Legacy fresh'); assert.strictEqual(buttons.reconcileButtonsFeatureState({ userId: 'legacy-fresh', target: { commentKey: 'ch-a:2', channelId: 'ch-a', postId: '2' } }).buttons.length, 1, 'BTN-064 canonical has one button');

    for (let i = 51; i <= 64; i += 1) console.log(`BTN-${String(i).padStart(3, '0')} pass`);
