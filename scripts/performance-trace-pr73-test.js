...
assert.ok(routes.includes('sanitizeMiniPayload'), 'routes should sanitize public timing payload');
assert.ok(routes.includes('STRING_LIMIT = 160'), 'routes should bound client supplied strings');
assert.ok(routes.includes('boundedNumber'), 'routes should bound client supplied numbers');
assert.ok(routes.includes('addPostPatchTraceHook'), 'routes should add, not replace, post patch trace hooks');
...
assert.ok(postPatcher.includes('postPatchTraceHooks = []'), 'postPatcher should support multiple trace hooks');
assert.ok(postPatcher.includes('function addPostPatchTraceHook'), 'postPatcher should expose additive trace hook');
assert.ok(postPatcher.includes('addPostPatchTraceHook'), 'postPatcher module export should include additive trace hook');
