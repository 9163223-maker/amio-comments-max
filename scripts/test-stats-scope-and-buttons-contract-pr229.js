...
(async()=>{
 ...
 assert.equal(statsData.loadStatsDataset({userId:'u-pr229',...allc},{period:'all'}).growth.joined,1);
 assert.equal(statsData.loadStatsDataset({userId:'u-pr229',...allch},{period:'all'}).growth.joined,1);
 const cost=statsData.writeManualCost({userId:'u-pr229',...channel},{source:'s',campaign:'c',amount:10},'added'); assert(statsData.getManualCosts({userId:'u-pr229',...channel},{period:'all'}).length===1); assert(statsData.getManualCosts({userId:'u-pr229',...channel2},{period:'all'}).length===0); assert(statsData.getManualCosts({userId:'u-pr229',...chat},{period:'all'}).length===0); assert(statsData.writeManualCost({userId:'u-pr229',...channel},{costId:cost.costId},'deleted'));
